...
def register_child_bot_db(token: str, owner_id: str, service_type: str) -> tuple[bool, str]:
    ...
    try:
        ...
        # Validate the token with Telegram API first
        test_bot = telebot.TeleBot(token)
        bot_info = test_bot.get_me()
        if not bot_info:
            return False, "Invalid bot token. Please check your token with @BotFather."

        # Insert the new bot document
        ...
        return False, "A bot with this token or for this service type is already registered."
    except telebot.apihelper.ApiTelegramException as e:
        logging.error(f"Invalid bot token provided for registration: {e}")
        return False, "Invalid bot token. Please check your token with @BotFather."
    except Exception as e:
        logging.error(f"Unexpected error registering child bot for owner {owner_id}, composite_key {composite_key}: {e}")
...
def handle_text_for_tts_or_mode_input_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    if current_voice:
        # Removed character limit check here - users can send any length text
        threading.Thread(
