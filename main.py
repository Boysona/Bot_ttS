...
# --- User state for input modes (kept in memory as transient) ---
...
def update_user_activity(user_id: int):
    """Update user's last_active timestamp in the database."""
...
def increment_processing_count(user_id: str, service_type: str):
    """Increment TTS or STT conversion count for a user."""
    field_to_inc = f"{service_type}_conversion_count"
    users_collection.update_one(
        {"_id": user_id},
        {"$inc": {field_to_inc: 1}, "$set": {"last_active": datetime.now().isoformat()}},
        upsert=True
    )

def get_tts_user_voice(user_id: str) -> str:
    """Get user's TTS voice from the database, defaulting to 'so-SO-MuuseNeural'."""
...
def set_tts_user_voice(user_id: str, voice: str):
    """Save TTS voice to the database."""
...
def get_stt_user_lang(user_id: str) -> str:
    """Get user's STT language from the database, defaulting to 'en'."""
...
def set_stt_user_lang(user_id: str, lang_code: str):
    """Save STT language to the database."""
...
def register_child_bot(token: str, owner_id: str, service_type: str):
    """Register a child bot in the database."""
    ...
    logging.info(f"Child bot {token[:5]}... registered for owner {owner_id} with service {service_type} in database.")
    return True

def get_child_bot_info(token: str) -> dict | None:
    """Retrieve child bot info from the database."""
    return registered_bots_collection.find_one({"_id": token})

def add_processing_stat(stat: dict):
    """Add a processing statistic to the database."""
...
def check_subscription(user_id: int) -> bool:
    """Verify if user is a member of the required channel."""
...
def send_subscription_message(chat_id: int):
    """Prompt user to join the required channel with the new text."""
...
4.  **Privacy & Data Handling**
    ...
    * **Your Settings are Saved:** To make your experience seamless, your chosen preferences (like TTS voice, pitch, rate, and STT language) are stored in our MongoDB database and persist across restarts.
...
def status_handler(message):
    ...
    active_today_count = users_collection.count_documents({"last_active": {"$gte": today_start}})
    total_registered_child_bots = registered_bots_collection.count_documents({})

    total_tts_conversions = processing_stats_collection.count_documents({"type": "tts"})
    total_stt_conversions = processing_stats_collection.count_documents({"type": "stt"})
    total_tts_proc_seconds = sum(doc["processing_time"] for doc in processing_stats_collection.find({"type": "tts"}))
    total_stt_proc_seconds = sum(doc["processing_time"] for doc in processing_stats_collection.find({"type": "stt"}))
    ...
    text = (
        ...
        f"▫️ Total Speech-to-Text Conversions: *{total_stt_conversions}*\n"
