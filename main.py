...
def send_broadcast_prompt(message):
    ...
    # Somali: Send the broadcast message now:
    # English: Send the broadcast message now:
...
def process_bot_token(message):
    ...
    if not (30 < len(bot_token) < 50 and ':' in bot_token):
        # Somali: Taasi ma aha Bot API Token sax ah. Fadlan hubi oo isku day mar kale.
        # English: That is not a valid Bot API Token. Please check and try again.
    ...
    try:
        ...
        markup.add(
            InlineKeyboardButton("Qoraal u beddel Cod (TTS)", callback_data="register_bot_service|tts"), # Somali: Convert Text to Audio (TTS)
            InlineKeyboardButton("Cod u beddel Qoraal (STT)", callback_data="register_bot_service|stt")  # Somali: Convert Audio to Text (STT)
        )
        # Somali: Fiican! Waxaan xaqiijiyay token-ka @**{bot_info.username}**.\n\nHadda dooro waxa botkaagu samayn doono:\n• **TTS Bot**: Qoraalka u beddela cod\n• **STT Bot**: Codka u beddela qoraal\n\nMid ka door hoose:
        # English: Great! I have verified the token for @**{bot_info.username}**.\n\nNow choose what your bot will do:\n• **TTS Bot**: Converts text to audio\n• **STT Bot**: Converts audio to text\n\nChoose one below:
        bot.send_message(message.chat.id,
        ...
        # Somali: ❌ Ma xaqiijin karo token-kaas. Waa laga yaabaa inuu khaldan yahay ama la tirtiray. Fadlan ka hubi @BotFather oo isku day mar kale.
        # English: ❌ I cannot verify that token. It might be incorrect or deleted. Please check with @BotFather and try again.
        bot.send_message(message.chat.id,
        ...
        # Somali: Cilad aan la filayn ayaa dhacday. Fadlan isku day mar kale.
...
def on_register_bot_service_select(call):
    ...
    if not data_state or data_state.get("state") != "awaiting_service_type":
        # Somali: Xaalad khaldan. Fadlan bilow mar kale `/reg`.
        # English: Invalid state. Please start again with `/reg`.
    ...
    if not bot_token:
        # Somali: Token lama helin. Bilow mar kale.
        # English: Token not found. Start again.
        bot.answer_callback_query(call.id, "Token not found. Start again.")
        # Somali: Waxbaa khaldamay. Isticmaal `/reg` mar kale.
        # English: Something went wrong. Use `/reg` again.
    ...
    try:
        ...
        # Somali: ✅ Botkaaga {service_type.upper()} waa la diiwaangeliyay!
        # English: ✅ Your {service_type.upper()} bot has been registered!
        bot.answer_callback_query(call.id, f"✅ Your {service_type.upper()} bot has been registered!")
        # Somali: 🎉 Botkaaga cusub *{service_type.upper()}* waa la dhaqaajiyay!\n\nKa hel halkan: https://t.me/{temp_child_bot.get_me().username}\n\nWaxay isticmaali doontaa xulashooyinkaaga (codka/pitch/rate ee TTS, luqadda ee STT) ee botkan weyn.\n• TTS: Isticmaal `/voice`, `/pitch`, `/rate`, oo qoraal soo dir\n• STT: Isticmaal `/lang` oo cod soo dir
        # English: 🎉 Your new *{service_type.upper()}* bot has been activated!\n\nFind it here: https://t.me/{temp_child_bot.get_me().username}\n\nIt will use your preferences (voice/pitch/rate for TTS, language for STT) from this main bot.\n• TTS: Use `/voice`, `/pitch`, `/rate`, and send text\n• STT: Use `/lang` and send audio
        bot.edit_message_text(
        ...
        # Somali: Ku guuldareystay in la dejiyo botkaaga. Isku day mar kale.
        # English: Failed to set up your bot. Try again.
        bot.answer_callback_query(call.id, "Failed to set up your bot. Try again.")
        # Somali: ❌ Cilad ayaa dhacday markii la dejinayay botkaaga. Isku day mar kale.
        # English: ❌ An error occurred while setting up your bot. Try again.
        ...
        # Somali: Cilad aan la filayn ayaa dhacday. Isku day mar kale.
        # English: An unexpected error occurred. Try again.
        bot.send_message(call.message.chat.id, "An unexpected error occurred. Try again.")
        # Somali: Cilad aan la filayn ayaa dhacday.
...
def make_pitch_keyboard():
    markup = InlineKeyboardMarkup(row_width=2)
    markup.add(
        InlineKeyboardButton("⬆️ Sare", callback_data="pitch_set|+50"), # Somali: ⬆️ Up
        InlineKeyboardButton("⬇️ Hoose", callback_data="pitch_set|-50"), # Somali: ⬇️ Down
        InlineKeyboardButton("🔄 Dib u deji", callback_data="pitch_set|0") # Somali: 🔄 Reset
    )
    return markup
...
def make_rate_keyboard():
    markup = InlineKeyboardMarkup(row_width=2)
    markup.add(
        InlineKeyboardButton("⚡️ Degdeg", callback_data="rate_set|+50"), # Somali: ⚡️ Fast
        InlineKeyboardButton("🐢 Gaabis", callback_data="rate_set|-50"), # Somali: 🐢 Slow
        InlineKeyboardButton("🔄 Dib u deji", callback_data="rate_set|0") # Somali: 🔄 Reset
    )
    return markup
...
def handle_pitch_command(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    # Somali: Aan hagaajino codka! Dooro mid ama soo dir nambar -100 (hoose) ilaa +100 (sare), 0 waa caadi:
    # English: Let's adjust the pitch! Choose one or send a number from -100 (low) to +100 (high), 0 is normal:
    target_bot.send_message(
        chat_id,
...
def handle_pitch_callback(call, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    try:
        ...
        # Somali: Codka waa {pitch_value}!
        # English: The pitch is {pitch_value}!
...
def handle_voice_command(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    # Somali: Marka hore, dooro *luqadda* codkaaga. 👇
    # English: First, choose the *language* of your voice. 👇
...
def handle_tts_language_select_callback(call, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    # Somali: Waa yahay! Hadda dooro *cod* gaar ah oo ka mid ah {lang_name}. 👇
    # English: Okay! Now choose a specific *voice* from {lang_name}. 👇
    target_bot.edit_message_text(
        chat_id=chat_id,
        message_id=call.message.message_id,
...
def handle_tts_voice_change_callback(call, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    # Somali: ✔️ Codka waa {voice}
    # English: ✔️ The voice is {voice}
    target_bot.answer_callback_query(call.id, f"✔️ The voice is {voice}")
    # Somali: 🔊 Wanaagsan! Waxaad isticmaalaysaa: *{voice}*.\n\nXulashooyinka hadda:\n• Codka: *{current_pitch}*\n• Xawaaraha: *{current_rate}*\n\nDiyaar ma u tahay inaad hadasho? Qoraal ii soo dir!
    # English: 🔊 Great! You are using: *{voice}*.\n\nCurrent settings:\n• Pitch: *{current_pitch}*\n• Rate: *{current_rate}*\n\nReady to speak? Send me text!
    target_bot.edit_message_text(
        chat_id=chat_id,
        message_id=call.message.message_id,
...
def handle_tts_back_to_languages_callback(call, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    # Somali: Dooro *luqadda* codkaaga. 👇
    # English: Choose the *language* of your voice. 👇
    target_bot.edit_message_text(
        chat_id=chat_id,
        message_id=call.message.message_id,
...
async def synth_and_send_tts(chat_id: int, user_id_for_settings: str, text: str, target_bot: telebot.TeleBot):
    ...
    try:
        ...
        if not os.path.exists(filename) or os.path.getsize(filename) == 0:
            # Somali: ❌ Waan ku guuldareystay inaan sameeyo codka. Isku day qoraal kale.
            # English: ❌ I failed to create the audio. Try another text.
            target_bot.send_message(chat_id, "❌ I failed to create the audio. Try another text.")
            return

        with open(filename, "rb") as f:
            # Somali: 🎧 *Waa kan codkaaga!* \n\nCodka: *{voice}*\nPitch: *{pitch}*\nXawaare: *{rate}*\n\nKu raaxayso dhageysiga! ✨
            # English: 🎧 *Here is your audio!* \n\nVoice: *{voice}*\nPitch: *{pitch}*\nRate: *{rate}*\n\nEnjoy listening! ✨
            target_bot.send_audio(
                chat_id,
                f,
                caption=f"🎧 *Here is your audio!* \n\n"
                        f"Voice: *{voice}*\n"
                        f"Pitch: *{pitch}*\n"
                        f"Rate: *{rate}*\n\n"
                        f"Enjoy listening! ✨",
        ...
        # Somali: ❌ Cilad ayaa dhacday markii la samaynayay codka. Isku day mar kale ama dooro cod kale.
        # English: ❌ An error occurred while generating the audio. Try again or choose another voice.
        target_bot.send_message(chat_id, "❌ An error occurred while generating the audio. Try again or choose another voice.")
    except Exception as e:
        logging.exception("TTS error")
        # Somali: Codkani ma heli karo, fadlan dooro mid kale.
        # English: This voice is not available, please choose another one.
...
def handle_language_stt_command(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    # Somali: Dooro *luqadda* ee qoraalkaaga Cod-u-Qoraal:
    # English: Choose the *language* for your Speech-to-Text:
...
async def process_stt_media(chat_id: int, user_id_for_settings: str, message_type: str, file_id: str, target_bot: telebot.TeleBot, original_message_id: int):
    ...
    try:
        ...
        if file_info.file_size > 20 * 1024 * 1024:
            # Somali: ⚠️ Faylka waa weyn yahay. Cabirka ugu badan waa 20MB. Soo dir fayl yar.
            # English: ⚠️ The file is too large. Maximum size is 20MB. Send a smaller file.
        ...
        if res['status'] == 'completed':
            text = res.get("text", "")
            if not text:
                # Somali: ℹ️ Ma jiro qoraal laga soo qaatay faylkan.
                # English: ℹ️ No text extracted from this file.
                ...
                # Somali: Qoraalkaagu waa dheer yahay. Waa kan faylka:
                # English: Your transcript is too long. Here is the file:
            ...
            # Somali: ❌ Cilad qoraal: {error_msg}
            # English: ❌ Transcription error: {error_msg}
        ...
        # Somali: ❌ Cilad shabakadeed ayaa dhacday. Isku day mar kale.
        # English: ❌ A network error occurred. Try again.
        ...
        # Somali: Faylka waa weyn yahay, soo dir mid ka yar 20MB.
        # English: The file is too large, send one smaller than 20MB.
...
def handle_stt_media_types_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    if message.voice:
        ...
        if message.document.mime_type and (message.document.mime_type.startswith('audio/') or message.document.mime_type.startswith('video/')):
            ...
            # Somali: Waan ka xumahay, kaliya waxaan qori karaa cod ama muuqaal. Soo dir fayl sax ah.
            # English: I'm sorry, I can only transcribe audio or video. Send a valid file.
            target_bot.send_message(message.chat.id, "I'm sorry, I can only transcribe audio or video. Send a valid file.")
            return

    if not file_id:
        # Somali: Nooca faylkan ma taageersani. Soo dir cod, fayl ama muuqaal.
        # English: I don't support this file type. Send audio, file or video.
    ...
    if user_id_for_settings not in in_memory_data["stt_settings"]:
        # Somali: ❗ Marka hore dooro luqadda qorida `/lang`.
        # English: ❗ First choose the transcription language `/lang`.
...
def handle_text_for_tts_or_mode_input_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    if user_rate_input_mode.get(user_id_for_settings) == "awaiting_rate_input":
        try:
            ...
            if -100 <= rate_val <= 100:
                ...
                # Somali: 🔊 Xawaaraha codka waa *{rate_val}*.
                # English: 🔊 The speech rate is *{rate_val}*.
                target_bot.send_message(message.chat.id, f"🔊 The speech rate is *{rate_val}*.", parse_mode="Markdown")
                user_rate_input_mode[user_id_for_settings] = None
            else:
                # Somali: ❌ Xawaare khaldan. Soo dir nambar -100 ilaa +100 ama 0. Isku day mar kale:
                # English: ❌ Invalid rate. Send a number from -100 to +100 or 0. Try again:
                target_bot.send_message(message.chat.id, "❌ Invalid rate. Send a number from -100 to +100 or 0. Try again:")
            return
        except ValueError:
            # Somali: Taasi maaha nambar sax ah. Soo dir nambar -100 ilaa +100 ama 0. Isku day mar kale:
            # English: That is not a valid number. Send a number from -100 to +100 or 0. Try again:
    ...
    if user_pitch_input_mode.get(user_id_for_settings) == "awaiting_pitch_input":
        try:
            ...
            if -100 <= pitch_val <= 100:
                ...
                # Somali: 🔊 Codka waa *{pitch_val}*.
                # English: 🔊 The pitch is *{pitch_val}*.
                target_bot.send_message(message.chat.id, f"🔊 The pitch is *{pitch_val}*.", parse_mode="Markdown")
                user_pitch_input_mode[user_id_for_settings] = None
            else:
                # Somali: ❌ Cod khaldan. Soo dir nambar -100 ilaa +100 ama 0. Isku day mar kale:
                # English: ❌ Invalid pitch. Send a number from -100 to +100 or 0. Try again:
    ...
    if current_voice:
        ...
        # Somali: Wali ma aadan dooran cod! Isticmaal `/voice` marka hore, ka dibna qoraal ii soo dir. 🗣️
        # English: You haven't chosen a voice yet! Use `/voice` first, then send me text. 🗣️
        target_bot.send_message(
            message.chat.id,
...
def handle_unsupported_media_types(message):
    ...
    # Somali: Waan ka xumahay, kaliya waxaan u beddeli karaa *qoraalka* cod ama *codka/faylka* qoraal. Soo dir mid ka mid ah!
    # English: I'm sorry, I can only convert *text* to audio or *audio/files* to text. Send me one of those!
    bot.send_message(
        message.chat.id,
            ...
            if message:
                ...
                if message.text and message.text.startswith('/start'):
                    if service_type == "stt":
                        ...
                        # Somali: 👋 Soo dhawoow! Waxaan ahay botkaaga {service_type.upper()}.
                        # English: 👋 Welcome! I am your {service_type.upper()} bot.
                ...
                if message.text:
                    if service_type == "tts":
                        ...
                        if message.text.startswith('/lang'):
                            ...
                            # Somali: Waxaan ahay STT bot. Soo dir cod, fayl ama muuqaal si aan u qoro, ama isticmaal `/lang`.
                            # English: I am an STT bot. Send audio, file or video for me to transcribe, or use `/lang`.
                    ...
                    if service_type == "stt":
                        ...
                        # Somali: Waxaan ahay TTS bot. Soo dir qoraal si aan u beddelo cod.
                        # English: I am a TTS bot. Send text for me to convert to audio.
                        child_bot_instance.send_message(chat_id, "I am a TTS bot. Send text for me to convert to audio.")
                    return "", 200
                else:
                    # Somali: Waan ka xumahay, waxaan ka shaqayn karaa noocyada fariimaha qaarkood. Eeg `/start`.
                    # English: I'm sorry, I can only work with certain message types. See `/start`.
                ...
                if service_type == "tts":
                    ...
                    if call.data.startswith("stt_lang|"):
                        ...
                        return "", 200
                # Somali: Tallaabadani ma heli karto nooca botkan.
                # English: This action is not available for this bot type.
...
def set_bot_commands():
    commands = [
        BotCommand("start", "Bilow"), # Somali: Start
        BotCommand("voice", "Dooro codka TTS"), # Somali: Choose TTS voice
        BotCommand("pitch", "Beddel codka"), # Somali: Change pitch
        BotCommand("rate", "Beddel xawaaraha"), # Somali: Change rate
        BotCommand("lang", "Dooro luqadda STT"), # Somali: Choose STT language
        BotCommand("reg", "Samee bot kaaga"), # Somali: Create your bot
        BotCommand("help", "Sida loo isticmaalo"), # Somali: How to use
    ]
    try:
...
def set_child_bot_commands(child_bot_instance: telebot.TeleBot, service_type: str):
    ...
    if service_type == "tts":
        commands = [
            BotCommand("start", "Bilow"), # Somali: Start
            BotCommand("voice", "Beddel codka TTS"), # Somali: Change TTS voice
            BotCommand("pitch", "Beddel codka"), # Somali: Change pitch
            BotCommand("rate", "Beddel xawaaraha") # Somali: Change rate
        ]
    elif service_type == "stt":
        commands = [
            BotCommand("start", "Bilow"), # Somali: Start
            BotCommand("lang", "Beddel luqadda") # Somali: Change language
        ]
    try:
