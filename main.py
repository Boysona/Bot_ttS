...
REQUIRED_CHANNEL = "@news_channals" # Transcriber Bot News Channel
...
premium_users = set()           # Set to store IDs of users allowed to use /register_bot

# Admin uptime message storage
admin_uptime_message = {}
admin_uptime_lock = threading.Lock()
admin_state = {} # Stores admin-specific states like 'awaiting_broadcast_message' or 'awaiting_user_id_for_premium'
...
def start_handler(message):
    ...
    admin_state[message.from_user.id] = None # Clear admin state if it was set for broadcast/accept user

    if message.from_user.id == ADMIN_ID:
        keyboard = telebot.types.ReplyKeyboardMarkup(resize_keyboard=True)
        keyboard.add("Send Broadcast", "Total Users", "/status", "Accept User") # Added Accept User button
...
def accept_user_prompt(message):
    admin_state[message.from_user.id] = 'awaiting_user_id_for_premium'
    bot.send_message(message.chat.id, "Please send the User ID of the user you want to grant premium access to (for /register_bot command):")

@bot.message_handler(
    func=lambda m: m.from_user.id == ADMIN_ID and admin_state.get(m.from_user.id) == 'awaiting_user_id_for_premium',
    content_types=['text']
)
def process_accept_user_id(message):
    admin_state[message.from_user.id] = None # Clear the admin state
    try:
        user_id_to_add = int(message.text.strip())
        premium_users.add(str(user_id_to_add)) # Add user ID to the premium set (as string for consistency with dict keys)
        bot.send_message(message.chat.id, f"User ID `{user_id_to_add}` has been successfully granted access to /register_bot command.", parse_mode="Markdown")
        logging.info(f"Admin {ADMIN_ID} granted premium access to user {user_id_to_add}.")
    except ValueError:
        bot.send_message(message.chat.id, "Invalid User ID. Please send a numeric ID.")
    except Exception as e:
        logging.error(f"Error processing accept user ID: {e}")
        bot.send_message(message.chat.id, "An error occurred while processing the User ID. Please try again.")
...
def register_bot_command(message):
    ...
    if uid == str(ADMIN_ID) or uid in premium_users:
        ...
        bot.send_message(message.chat.id,
                         "The /register_bot command is reserved for premium users. "
                         "If you'd like to create a bot like this (without coding or hosting), please contact the developer: @user33230.")
...
async def synth_and_send_tts(chat_id: int, user_id_for_settings: str, text: str, target_bot: telebot.TeleBot):
    ...
    try:
        # Send processing message with the new text
        processing_message = target_bot.send_message(chat_id, "⌛️ Processing... \n\nDo you want a bot like this? \n\nCreate your bot 👉 @Bot_BuilderrBot \n\nPowered by @Bot_BuilderrBot")
        ...
        if processing_message:
            try:
                target_bot.delete_message(chat_id, processing_message.message_id)
            except Exception as e:
                logging.error(f"Could not delete processing message for TTS: {e}")
...
async def process_stt_media(chat_id: int, user_id_for_settings: str, message_type: str, file_id: str, target_bot: telebot.TeleBot):
    ...
    processing_msg = None
    try:
        processing_msg = target_bot.send_message(chat_id, "⌛️ Processing... \n\nDo you want a bot like this? \n\nCreate your bot 👉 @Bot_BuilderrBot \n\nPowered by @Bot_BuilderrBot") # Updated here
        ...
        if res['status'] == 'completed':
            ...
            elif len(text) <= 4096: # Telegram message limit for text
...
def handle_text_for_tts_or_mode_input_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    if current_voice:
        # Removed the 1000 character limit. MSSpeech handles internal limits.
        # Max text length for Telegram message is 4096 characters, so keep it within that.
        if len(message.text) > 4096:
            target_bot.send_message(message.chat.id, "Your text is too long. Please keep it under 4096 characters for text-to-speech conversion.")
            return
                ...
                if message.text and message.text.startswith('/start'):
                    if service_type == "tts":
                        ...
                        welcome_message = (
                            ...
                            "• Or click /voice\n"
                            "• For more commands, go to the Menu"
                        )
                    elif service_type == "stt":
                        current_lang_code = get_stt_user_lang_in_memory(user_id_for_settings)
...
def set_bot_commands():
    ...
    commands = [
        ...
        #BotCommand("privacy", "🔒 Read privacy notice"), # Re-add if needed
