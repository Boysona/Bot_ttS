...
def register_child_bot(token: str, owner_id: str, service_type: str):
    """
    Registers a new child bot in MongoDB, or updates it if already registered.
    Returns True on success (either new registration or token already existed),
    False only on a MongoDB operation failure.
    """
    try:
        ...
        if existing_bot:
            ...
            # then consider it a success and return True.
            if existing_bot.get("owner_id") != owner_id or existing_bot.get("service_type") != service_type:
                registered_bots_collection.update_one(
                    {"_id": token},
                    {"$set": {"owner_id": owner_id, "service_type": service_type}}
                )
                logging.info(f"Child bot {token[:5]}... updated for owner {owner_id} with service {service_type} in MongoDB.")
            else:
                logging.info(f"Child bot {token[:5]}... already registered and up-to-date for owner {owner_id} with service {service_type}.")
            return True # Token already exists and is considered "OK"

        # If not already registered, insert new
        ...
        logging.info(f"Child bot {token[:5]}... newly registered for owner {owner_id} with service {service_type} in MongoDB.")
        return True
    except OperationFailure as e:
        logging.error(f"MongoDB operation failed for register_child_bot: {e}")
        return False # Only return False if there's a database error
...
def on_register_bot_service_select(call):
    ...
    # Call the modified register_child_bot function
    if register_child_bot(bot_token, uid, service_type):
        ...
        # This branch will now only be hit if there's an actual MongoDB operation failure
        bot.answer_callback_query(call.id, "Failed to register your bot due to a database error. Please try again.")
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                              text="Failed to register your bot due to a database error. Please try again later.")
