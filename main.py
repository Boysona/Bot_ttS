...
MONGO_URI = "mongodb+srv://hoskasii:GHyCdwpI0PvNuLTjQ@cluster0.dy7oe7t.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
...
# IMPORTANT: These must be keyed by the *interacting user's ID*, not the bot's owner_id
...
user_register_bot_mode = {}     # { user_id: "awaiting_token" or "awaiting_service_type" } (only for main bot)
...
def start_handler(message):
    ...
    # Ensure all input modes are OFF on /start for the interacting user
...
def help_handler(message):
    ...
    # Ensure all input modes are OFF on /help for the interacting user
...
def privacy_notice_handler(message):
    ...
    # Ensure all input modes are OFF on /privacy for the interacting user
    user_tts_mode[user_id] = None
    user_pitch_input_mode[user_id] = None
    user_rate_input_mode[user_id] = None
    user_register_bot_mode[user_id] = None # Clear this mode
...
def status_handler(message):
    ...
    # Ensure all input modes are OFF on /status for the interacting user
    user_tts_mode[user_id] = None
    user_pitch_input_mode[user_id] = None
    user_rate_input_mode[user_id] = None
    user_register_bot_mode[user_id] = None # Clear this mode
...
def register_bot_command(message):
    ...
    # Clear other modes for the interacting user
...
def on_register_bot_service_select(call):
    ...
    if register_child_bot_db(bot_token, uid, service_type): # uid here is the owner_id for the child bot
        try:
            ...
            bot.edit_message_text(
                ...
                text=f"🎉 Your new *{service_type.upper()} Bot* is now active!\n\n"
                     ...
                     f"It will use *each user's individual settings* (voice/pitch/rate for TTS, language for STT) "
                     f"which are managed by this main bot. No new server or code needed! Just start interacting with it.",
                parse_mode="Markdown"
            )
...
def handle_rate_command(message, target_bot: telebot.TeleBot, interacting_user_id: str):
    """Handles the /rate command for both main and child bots."""
    chat_id = message.chat.id
    # Use interacting_user_id for state management
    user_id_for_state = interacting_user_id

    # Clear other modes for this user_id
    ...
    # user_register_bot_mode is only for main bot's owner, not cleared here for interacting users
...
def handle_rate_callback(call, target_bot: telebot.TeleBot, interacting_user_id: str):
    ...
    try:
        ...
        user_rate_input_mode[user_id_for_state] = None # Clear the input mode
        ...
        logging.error(f"Error setting rate from callback for user {interacting_user_id}: {e}")
...
def handle_pitch_command(message, target_bot: telebot.TeleBot, interacting_user_id: str):
...
def handle_pitch_callback(call, target_bot: telebot.TeleBot, interacting_user_id: str):
    ...
    try:
        ...
        user_pitch_input_mode[user_id_for_state] = None # Clear the input mode
        ...
        logging.error(f"Error setting pitch from callback for user {interacting_user_id}: {e}")
        target_bot.answer_callback_query(call.id, "An error occurred.")
...
def handle_voice_command(message, target_bot: telebot.TeleBot, interacting_user_id: str):
...
def handle_tts_language_select_callback(call, target_bot: telebot.TeleBot, interacting_user_id: str):
...
def handle_tts_voice_change_callback(call, target_bot: telebot.TeleBot, interacting_user_id: str):
...
def handle_tts_back_to_languages_callback(call, target_bot: telebot.TeleBot, interacting_user_id: str):
...
async def synth_and_send_tts(chat_id: int, user_id: str, text: str, target_bot: telebot.TeleBot):
    ...
    `user_id` is always the interacting user's user_id for settings.
    """
...
def handle_language_stt_command(message, target_bot: telebot.TeleBot, interacting_user_id: str):
...
def handle_stt_language_select_callback(call, target_bot: telebot.TeleBot, interacting_user_id: str):
...
def handle_stt_media_types_common(message, target_bot: telebot.TeleBot, interacting_user_id: str):
    """
    Common handler for STT media types, used by both main and child bots.
    `interacting_user_id` is the ID of the user who sent the media.
    """
    update_user_activity_db(int(interacting_user_id)) # Update the interacting user's activity

    # Clear all modes when media is sent, as we assume user wants STT
    user_tts_mode[interacting_user_id] = None
    user_pitch_input_mode[interacting_user_id] = None
    user_rate_input_mode[interacting_user_id] = None
    # user_register_bot_mode is only for main bot's owner, not cleared here for interacting users
    ...
    if interacting_user_id not in _stt_lang_cache:
        target_bot.send_message(message.chat.id, "❗ Please choose a language for transcription first using /language_stt.")
        return

    threading.Thread(
        target=lambda: asyncio.run(process_stt_media(message.chat.id, interacting_user_id, message_type, file_id, target_bot))
    ).start()
...
def handle_text_for_tts_or_mode_input_common(message, target_bot: telebot.TeleBot, interacting_user_id: str):
    """
    Common handler for text messages, used by both main and child bots.
    `interacting_user_id` is the ID of the user who sent the text.
    """
    update_user_activity_db(int(interacting_user_id))
    ...
    if user_rate_input_mode.get(interacting_user_id) == "awaiting_rate_input":
        try:
            rate_val = int(message.text)
            if -100 <= rate_val <= 100:
                set_tts_user_rate_db(interacting_user_id, rate_val)
                target_bot.send_message(message.chat.id, f"🔊 Voice speed set to *{rate_val}*.", parse_mode="Markdown")
                user_rate_input_mode[interacting_user_id] = None # Reset the state
            else:
    ...
    if user_pitch_input_mode.get(interacting_user_id) == "awaiting_pitch_input":
        try:
            pitch_val = int(message.text)
            if -100 <= pitch_val <= 100:
                set_tts_user_pitch_db(interacting_user_id, pitch_val)
                target_bot.send_message(message.chat.id, f"🔊 Voice pitch set to *{pitch_val}*.", parse_mode="Markdown")
                user_pitch_input_mode[interacting_user_id] = None # Reset the state
            else:
    ...
    current_voice = get_tts_user_voice_db(interacting_user_id)
    ...
    if current_voice:
        ...
        threading.Thread(
            target=lambda: asyncio.run(synth_and_send_tts(message.chat.id, interacting_user_id, message.text, target_bot))
        ).start()
    else:
...
def handle_unsupported_media_types(message):
    ...
    # Clear all input modes for the interacting user, as this is likely a misfire
    user_tts_mode[uid] = None
    user_pitch_input_mode[uid] = None
    user_rate_input_mode[uid] = None
    # user_register_bot_mode is only for main bot's owner, not cleared here for interacting users
...
def child_webhook(child_bot_token):
    if request.method == "POST":
        ...
        if content_type and content_type.startswith("application/json"):
            ...
            # owner_id = bot_info["owner_id"] # This is the bot's owner, NOT the interacting user.
            ...
            if message:
                chat_id = message.chat.id
                # Get the ID of the user interacting with this child bot
                interacting_user_id = str(message.from_user.id)
                ...
                if message.text and message.text.startswith('/start'):
                    ...
                    if service_type == "tts":
                        current_voice = get_tts_user_voice_db(interacting_user_id) # Get settings for the interacting user
                        current_pitch = get_tts_user_pitch_db(interacting_user_id) # Get settings for the interacting user
                        current_rate = get_tts_user_rate_db(interacting_user_id)   # Get settings for the interacting user
                        welcome_message += (
                        ...
                        current_lang_code = get_stt_user_lang_db(interacting_user_id) # Get settings for the interacting user
                ...
                if message.text:
                    if service_type == "tts":
                        if message.text.startswith('/voice'):
                            handle_voice_command(message, child_bot_instance, interacting_user_id) # Pass interacting_user_id
                            return "", 200
                        elif message.text.startswith('/pitch'):
                            handle_pitch_command(message, child_bot_instance, interacting_user_id) # Pass interacting_user_id
                            return "", 200
                        elif message.text.startswith('/rate'):
                            handle_rate_command(message, child_bot_instance, interacting_user_id)   # Pass interacting_user_id
                            return "", 200
                        else:
                            # If it's not a command, process as text for TTS
                            handle_text_for_tts_or_mode_input_common(message, child_bot_instance, interacting_user_id) # Pass interacting_user_id
                            return "", 200
                    elif service_type == "stt":
                        if message.text.startswith('/language_stt'):
                            handle_language_stt_command(message, child_bot_instance, interacting_user_id) # Pass interacting_user_id
                            return "", 200
                        else:
                    ...
                    if service_type == "stt":
                        handle_stt_media_types_common(message, child_bot_instance, interacting_user_id) # Pass interacting_user_id
                    else:
                ...
                chat_id = call.message.chat.id
                # Get the ID of the user interacting with this child bot
                interacting_user_id = str(call.from_user.id)
                
                if service_type == "tts":
                    if call.data.startswith("tts_lang|"):
                        handle_tts_language_select_callback(call, child_bot_instance, interacting_user_id) # Pass interacting_user_id
                        return "", 200
                    elif call.data.startswith("tts_voice|"):
                        handle_tts_voice_change_callback(call, child_bot_instance, interacting_user_id) # Pass interacting_user_id
                        return "", 200
                    elif call.data == "tts_back_to_languages":
                        handle_tts_back_to_languages_callback(call, child_bot_instance, interacting_user_id) # Pass interacting_user_id
                        return "", 200
                    elif call.data.startswith("pitch_set|"):
                        handle_pitch_callback(call, child_bot_instance, interacting_user_id) # Pass interacting_user_id
                        return "", 200
                    elif call.data.startswith("rate_set|"):
                        handle_rate_callback(call, child_bot_instance, interacting_user_id)   # Pass interacting_user_id
                        return "", 200
                elif service_type == "stt":
                    if call.data.startswith("stt_lang|"):
                        handle_stt_language_select_callback(call, child_bot_instance, interacting_user_id) # Pass interacting_user_id
                        return "", 200
