...
registered_bots_collection = None
...
def connect_to_mongodb():
    ...
    """
    global mongo_client, db, users_collection, tts_settings_collection, stt_settings_collection, registered_bots_collection, processing_stats_collection
    try:
...
# --- BOT CONFIGURATION ---
TOKEN = "7790991731:AAFgEjc6O-iTSSkpt3lEJBH86gQY5nIgAw"  # <-- Main Bot Token
ADMIN_ID = 5978150981
...
def register_child_bot(token: str, owner_id: str, service_type: str):
    """
    Registers a new child bot in MongoDB, or confirms existence if already registered.
    Returns True if the bot is successfully registered or already exists, False on a database error.
    """
    try:
        # Check if already registered
        existing_bot = registered_bots_collection.find_one({"_id": token})
        if existing_bot:
            logging.info(f"Child bot with token {token[:5]}... already registered. Confirming existence.")
            return True # Bot already exists, treat as success for registration purposes

        # If not already registered, insert new bot
        ...
        logging.info(f"Child bot {token[:5]}... registered for owner {owner_id} with service {service_type} in MongoDB.")
        return True
    except OperationFailure as e:
        logging.error(f"MongoDB operation failed for register_child_bot: {e}")
        return False
...
def get_child_bot_info(token: str) -> dict | None:
    ...
    try:
        ...
        logging.error(f"MongoDB operation failed for get_child_bot_info: {e}")
        return None
...
def on_register_bot_service_select(call):
    ...
    # The register_child_bot function now handles checking if it already exists
    if register_child_bot(bot_token, uid, service_type):
        try:
            ...
            # It's important to set drop_pending_updates=True here if the bot is being (re)configured
            # to ensure it starts with a clean slate for its new service type/webhook.
            # If the token was already registered, this essentially "restarts" it.
            temp_child_bot.set_webhook(url=child_bot_webhook_url, drop_pending_updates=True)

            # Set commands for the newly registered/confirmed child bot
        ...
        bot.answer_callback_query(call.id, "Failed to register your bot. This is likely a database issue. Please contact support.")
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                              text="Failed to register your bot due to a database error. Please try again later or contact support.")
...
def child_webhook(child_bot_token):
    if request.method == "POST":
        ...
        if content_type and content_type.startswith("application/json"):
            ...
            bot_info = get_child_bot_info(child_bot_token) # Retrieve from MongoDB
            if not bot_info:
...
def set_webhook_on_startup():
    try:
        ...
        # Also, set webhooks for all previously registered child bots on startup (from MongoDB)
        try:
            for bot_doc in registered_bots_collection.find({}):
                ...
                try:
                    # Use drop_pending_updates=True here when re-setting webhooks on startup
                    # to prevent processing old updates if the bot was down or webhook was misconfigured.
            ...
            logging.error(f"MongoDB operation failed when retrieving registered bots on startup: {e}")
