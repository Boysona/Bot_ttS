...
# --- In-memory data storage (for child bots only, as per requirement) ---
in_memory_data = {
...
user_register_bot_mode = {}     # { user_id: "awaiting_token" or "awaiting_service_type" or {"state": "awaiting_service_type", "token": "..."} }
...
def save_user_data_to_db(user_data: dict) -> bool:
    ...
    try:
        ...
        logging.error(f"MongoDB error saving user data: {e}")
        return False

def get_user_data_from_db(user_id: str) -> dict | None:
    """Retrieves user data from MongoDB."""
    if not db: return False
    try:
        return db.users.find_one({"_id": str(user_id)})
    except PyMongoError as e:
        logging.error(f"MongoDB error getting user data: {e}")
        return None

def update_user_activity_in_db(user_id: int):
    """Update user.last_active = now() and ensure user exists in DB."""
    user_id_str = str(user_id)
    now_iso = datetime.now().isoformat()
    user_doc = get_user_data_from_db(user_id_str)
    ...
    if not user_doc:
        ...
        user_doc["last_active"] = now_iso
    save_user_data_to_db(user_doc)

def increment_processing_count_in_db(user_id: str, service_type: str):
    """Increment either tts_conversion_count or stt_conversion_count in DB."""
    user_id_str = str(user_id)
    now_iso = datetime.now().isoformat()
    
    # Ensure user exists and has default counts if not present
    user_doc = get_user_data_from_db(user_id_str)
    if not user_doc:
        user_doc = {
            "_id": user_id_str,
    ...
    user_doc[field_to_inc] = user_doc.get(field_to_inc, 0) + 1
    user_doc["last_active"] = now_iso # Also update last active
    save_user_data_to_db(user_doc)


def save_tts_settings_to_db(user_id: str, settings: dict) -> bool:
    """Saves or updates TTS settings for a user in MongoDB."""
    if not db: return False
    try:
        db.tts_settings.update_one(
            {"_id": str(user_id)},
            {"$set": settings},
            upsert=True
        )
        ...
        logging.error(f"MongoDB error saving TTS settings: {e}")
        return False

def get_tts_settings_from_db(user_id: str) -> dict | None:
    """Retrieves TTS settings for a user from MongoDB."""
    if not db: return None
    try:
        return db.tts_settings.find_one({"_id": str(user_id)})
    except PyMongoError as e:
        logging.error(f"MongoDB error getting TTS settings: {e}")
        return None

def get_tts_user_voice_from_db(user_id: str) -> str:
    """Return TTS voice from DB (default "so-SO-MuuseNeural")."""
    settings = get_tts_settings_from_db(user_id)
    return settings.get("voice", "so-SO-MuuseNeural") if settings else "so-SO-MuuseNeural"

def set_tts_user_voice_in_db(user_id: str, voice: str):
    """Save TTS voice in DB."""
    settings = get_tts_settings_from_db(user_id) or {}
    settings["voice"] = voice
    save_tts_settings_to_db(user_id, settings)

def get_tts_user_pitch_from_db(user_id: str) -> int:
    """Return TTS pitch from DB (default 0)."""
    settings = get_tts_settings_from_db(user_id)
    return settings.get("pitch", 0) if settings else 0

def set_tts_user_pitch_in_db(user_id: str, pitch: int):
    """Save TTS pitch in DB."""
    settings = get_tts_settings_from_db(user_id) or {}
    settings["pitch"] = pitch
    save_tts_settings_to_db(user_id, settings)

def get_tts_user_rate_from_db(user_id: str) -> int:
    """Return TTS rate from DB (default 0)."""
    settings = get_tts_settings_from_db(user_id)
    return settings.get("rate", 0) if settings else 0

def set_tts_user_rate_in_db(user_id: str, rate: int):
    """Save TTS rate in DB."""
    settings = get_tts_settings_from_db(user_id) or {}
    settings["rate"] = rate
    save_tts_settings_to_db(user_id, settings)
...
def save_stt_settings_to_db(user_id: str, settings: dict) -> bool:
    """Saves or updates STT settings for a user in MongoDB."""
    if not db: return False
    try:
        db.stt_settings.update_one(
            {"_id": str(user_id)},
            {"$set": settings},
            upsert=True
        )
        ...
        logging.error(f"MongoDB error saving STT settings: {e}")
        return False

def get_stt_settings_from_db(user_id: str) -> dict | None:
    """Retrieves STT settings for a user from MongoDB."""
    if not db: return None
    try:
        return db.stt_settings.find_one({"_id": str(user_id)})
    except PyMongoError as e:
        logging.error(f"MongoDB error getting STT settings: {e}")
        return None

def get_stt_user_lang_from_db(user_id: str) -> str:
    """Return STT language from DB (default "en")."""
    settings = get_stt_settings_from_db(user_id)
    return settings.get("language_code", "en") if settings else "en"

def set_stt_user_lang_in_db(user_id: str, lang_code: str):
    """Save STT language in DB."""
    settings = get_stt_settings_from_db(user_id) or {}
    settings["language_code"] = lang_code
    save_stt_settings_to_db(user_id, settings)

def add_processing_stat_to_db(stat: dict):
    """Adds a processing statistic entry to MongoDB."""
    if not db: return False
    try:
        db.processing_stats.insert_one(stat)
        return True
    except PyMongoError as e:
        logging.error(f"MongoDB error adding processing stat: {e}")
        return False

def get_total_users_from_db() -> int:
    """Returns total registered users from MongoDB."""
    if not db: return 0
    try:
        return db.users.count_documents({})
    except PyMongoError as e:
        logging.error(f"MongoDB error counting total users: {e}")
        return 0

def get_active_users_today_from_db() -> int:
    """Returns count of users active today from MongoDB."""
    if not db: return 0
    try:
        today_iso_prefix = datetime.now().date().isoformat()
        return db.users.count_documents({"last_active": {"$regex": f"^{today_iso_prefix}"}})
    except PyMongoError as e:
        logging.error(f"MongoDB error counting active users today: {e}")
        return 0

def get_total_processing_stats_from_db() -> dict:
    """Returns aggregated processing stats from MongoDB."""
    if not db: return {}
    try:
        pipeline = [
            {"$group": {
        ...
        results = list(db.processing_stats.aggregate(pipeline))
        ...
        for res in results:
            if res["_id"] == "tts":
                stats["tts_conversions"] = res["total_conversions"]
                stats["tts_processing_time"] = res["total_processing_time"]
            elif res["_id"] == "stt":
                stats["stt_conversions"] = res["total_conversions"]
                stats["stt_processing_time"] = res["total_processing_time"]
        return stats
    except PyMongoError as e:
        logging.error(f"MongoDB error aggregating processing stats: {e}")
        return {}


# --- New: Register/Get Child Bot Info (in-memory as per requirement) ---
...
def start_handler(message):
    ...
    update_user_activity_in_db(message.from_user.id)
    ...
    if message.from_user.id == ADMIN_ID:
        ...
        keyboard.add("Send Broadcast", "Total Users", "Processing Stats", "Uptime")
...
4.  **Privacy & Data Handling**
    ...
    * **Your Settings are Saved (in MongoDB):** To make your experience seamless, your chosen preferences (like selected TTS voice, pitch, rate, and STT language) are stored securely in our database.
    * **Basic Activity Logging:** We also record basic activity (such as your last active timestamp and usage counts) for anonymous, aggregated statistics to help us improve the service.
...
Your privacy is important to us. Here's a quick overview of how we handle your data:

* **Text and Media Content:** Any text you send for Text-to-Speech (TTS) and any voice, audio, or video files you send for Speech-to-Text (STT) are processed in real-time and **are NOT stored** on our servers. Once the conversion/transcription is complete and the output is sent back to you, the content is immediately deleted from our processing systems. We do not retain copies of your private conversations or media files.

* **User Preferences (TTS/STT Settings):** To provide a consistent experience, your chosen preferences such as your selected TTS voice, pitch, rate, and STT language are saved in our database. This ensures that when you return to the bot, your settings are remembered. These settings do not contain any personally identifiable information beyond what is necessary to link them to your Telegram user ID.

* **Basic Activity Data:** We collect anonymous, aggregated statistics on bot usage to understand overall performance and improve our services. This includes:
    * Your Telegram User ID (used as a unique identifier, but not linked to your personal Telegram account details beyond what Telegram provides to bots).
    * Your last active timestamp.
    * Counts of TTS and STT conversions performed.
    This data is used solely for internal analytics and service improvement, not for tracking individual user behavior or for marketing purposes.

* **No Sharing with Third Parties:** We do not sell, rent, or share your personal data or content with third parties. All processing is done securely through reputable API providers (like AssemblyAI for STT and Microsoft Azure for TTS) under strict data privacy agreements.

* **Security:** We implement reasonable security measures to protect the data we collect. However, no method of transmission over the Internet or method of electronic storage is 100% secure.
...
@bot.message_handler(func=lambda m: m.text == "Uptime" and m.from_user.id == ADMIN_ID)
def uptime_admin_handler(message):
    ...
    with admin_uptime_lock:
        if (
            ...
            # If already running, stop the old one to start a new one with correct message_id
            admin_uptime_message[ADMIN_ID]['thread'].join(timeout=1) # Give it a moment to stop
            del admin_uptime_message[ADMIN_ID]['thread'] # Clear the reference
...
def total_users(message):
    total_registered = get_total_users_from_db()
    active_today = get_active_users_today_from_db()

    text = (
        "👥 *User Statistics*\n"
        f"▫️ Total Registered Users: *{total_registered}*\n"
        f"▫️ Active Users Today: *{active_today}*\n"
        f"▫️ Total Registered Child Bots (in-memory): *{len(in_memory_data['registered_bots'])}*\n"
    )
    bot.send_message(message.chat.id, text, parse_mode="Markdown")

@bot.message_handler(func=lambda m: m.text == "Processing Stats" and m.from_user.id == ADMIN_ID)
def processing_stats_handler(message):
    stats = get_total_processing_stats_from_db()
    
    tts_proc_seconds = stats.get("tts_processing_time", 0)
    stt_proc_seconds = stats.get("stt_processing_time", 0)

    tts_proc_hours = int(tts_proc_seconds) // 3600
    tts_proc_minutes = (int(tts_proc_seconds) % 3600) // 60
    tts_proc_seconds = int(tts_proc_seconds) % 60

    stt_proc_hours = int(stt_proc_seconds) // 3600
    stt_proc_minutes = (int(stt_proc_seconds) % 3600) // 60
    stt_proc_seconds = int(stt_proc_seconds) % 60

    text = (
        "⚙️ *Processing Statistics*\n"
        f"▫️ Total Text-to-Speech Conversions: *{stats.get('tts_conversions', 0)}*\n"
        f"⏱️ Total TTS Processing Time: *{tts_proc_hours} hours {tts_proc_minutes} minutes {tts_proc_seconds} seconds*\n"
        f"▫️ Total Speech-to-Text Conversions: *{stats.get('stt_conversions', 0)}*\n"
...
def broadcast_message(message):
    ...
    if db:
        for user_doc in db.users.find({}, {"_id": 1}): # Project only _id
            uid = user_doc["_id"]
            if uid == str(ADMIN_ID):
                continue
            try:
...
def process_bot_token(message):
    ...
    # Check if this token is already registered (in-memory, as per requirement)
    if get_child_bot_info_in_memory(bot_token):
        bot.send_message(message.chat.id,
                         "This bot token is already registered. You can start using it if you know its service type. If you need to change its service, please contact support.")
        user_register_bot_mode[uid] = None # Clear state after informing
        return
...
def on_register_bot_service_select(call):
    ...
    # Register the bot in-memory (as per requirement)
    if register_child_bot_in_memory(bot_token, uid, service_type):
        ...
        # This branch should now be hit if the token was already registered in-memory
...
def handle_rate_callback(call, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    try:
        ...
        set_tts_user_rate_in_db(user_id_for_settings, rate_value)
...
def on_rate_set_callback(call):
    uid = str(call.from_user.id)
    update_user_activity_in_db(call.from_user.id)
...
def handle_pitch_callback(call, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    try:
        ...
        set_tts_user_pitch_in_db(user_id_for_settings, pitch_value)
...
def handle_tts_voice_change_callback(call, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    set_tts_user_voice_in_db(user_id_for_settings, voice)

    user_tts_mode[user_id_for_settings] = voice # Update mode with selected voice

    current_pitch = get_tts_user_pitch_from_db(user_id_for_settings)
    current_rate = get_tts_user_rate_from_db(user_id_for_settings)
...
async def synth_and_send_tts(chat_id: int, user_id_for_settings: str, text: str, target_bot: telebot.TeleBot):
    ...
    voice = get_tts_user_voice_from_db(user_id_for_settings)
    pitch = get_tts_user_pitch_from_db(user_id_for_settings)
    rate = get_tts_user_rate_from_db(user_id_for_settings)
    ...
    try:
        ...
        increment_processing_count_in_db(user_id_for_settings, "tts") # Increment user's TTS count

        add_processing_stat_to_db({
...
def handle_stt_language_select_callback(call, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    set_stt_user_lang_in_db(user_id_for_settings, lang_code)
...
async def process_stt_media(chat_id: int, user_id_for_settings: str, message_type: str, file_id: str, target_bot: telebot.TeleBot, original_message_id: int): # Added original_message_id
    ...
    try:
        ...
        lang_code = get_stt_user_lang_from_db(user_id_for_settings)
        ...
        if res['status'] == 'completed':
            ...
            increment_processing_count_in_db(user_id_for_settings, "stt") # Increment user's STT count
            status = "success"
        else:
        ...
        add_processing_stat_to_db({
            ...
            "language_code": get_stt_user_lang_from_db(user_id_for_settings),
            "error_message": str(e)
        })
...
def handle_stt_media_types_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    """
    update_user_activity_in_db(int(user_id_for_settings)) # Update this user's activity
    ...
    stt_settings = get_stt_settings_from_db(user_id_for_settings)
    if not stt_settings or "language_code" not in stt_settings:
...
def handle_text_for_tts_or_mode_input_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    `user_id_for_settings` is the actual user's ID for settings lookup and activity updates.
    """
    update_user_activity_in_db(int(user_id_for_settings))
    ...
    if user_rate_input_mode.get(user_id_for_settings) == "awaiting_rate_input":
        try:
            ...
            if -100 <= rate_val <= 100:
                set_tts_user_rate_in_db(user_id_for_settings, rate_val)
    ...
    if user_pitch_input_mode.get(user_id_for_settings) == "awaiting_pitch_input":
        try:
            ...
            if -100 <= pitch_val <= 100:
                set_tts_user_pitch_in_db(user_id_for_settings, pitch_val)
    ...
    current_voice = get_tts_user_voice_from_db(user_id_for_settings)
        ...
        if content_type and content_type.startswith("application/json"):
            ...
            if message:
                ...
                if message.text and message.text.startswith('/start'):
                    # Update user activity in DB using their actual ID
                    update_user_activity_in_db(message.from_user.id)
...
def set_webhook_on_startup():
    try:
        ...
        # Note: In a production scenario, you'd likely load registered bots from DB here too.
        # But per requirements, child bot info is in-memory.
...
def set_bot_info_and_startup():
    ...
    if not connect_to_mongodb(): # Attempt to connect to MongoDB
        logging.error("Failed to connect to MongoDB on startup. Bot will run with limited functionality (no persistent data).")
