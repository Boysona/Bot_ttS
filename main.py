...
# --- BOT CONFIGURATION ---
TOKEN = "8089409453:AAH-DjTRVY1H5kMp_ymE6ThYAl4zC4BGTxM"  # <-- Main Bot Token
ADMIN_ID = 5978150981
...
def connect_to_mongodb():
    ...
    try:
        ...
        registered_bots_collection.create_index([("token", ASCENDING)], unique=True) # New index on token
        registered_bots_collection.create_index([("owner_id", ASCENDING), ("service_type", ASCENDING)], unique=True) # New index on owner_id and service_type
...
def register_child_bot_db(token: str, owner_id: str, service_type: str) -> tuple[bool, str]:
    """
    ...
    try:
        # Check if a bot with this token already exists regardless of owner/service type
        existing_bot_with_token = registered_bots_collection.find_one({"token": token})
        if existing_bot_with_token and existing_bot_with_token["_id"] != composite_key:
            return False, "This bot token is already registered with another bot or service type."

        # Check for existing composite key (owner_id_service_type) specifically
        existing_bot_for_service = registered_bots_collection.find_one({"_id": composite_key})
        if existing_bot_for_service:
            return False, f"You already have a {service_type.upper()} bot registered."
        ...
        logging.error(f"MongoDB connection failed during child bot registration for {composite_key}: {e}")
        return False, f"Database connection error: {e}"
    except DuplicateKeyError as e: # This specifically catches the unique index violation
        logging.error(f"Duplicate key error during child bot registration for {composite_key}: {e}")
        return False, "A bot with this token or for this service type is already registered. Please use a unique token or check your existing bots."
    except Exception as e:
        logging.error(f"Error registering child bot for owner {owner_id}, composite_key {composite_key}: {e}")
...
def on_register_bot_service_select(call):
    ...
    success, error_message = register_child_bot_db(bot_token, uid, service_type)
    ...
    if success:
        ...
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                              text=f"Failed to register your bot: *{error_message}* Please try again later.",
                              parse_mode="Markdown")
