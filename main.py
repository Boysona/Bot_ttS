...
MONGO_URI = "mongodb+srv://hoskasii:GHyCdwpI0PvNuLTj@cluster0.dy7oe7t.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
...
def get_tts_settings_collection():
    return db["tts_settings"]

def get_stt_settings_collection():
    return db["stt_settings"]

def get_processing_stats_collection():
    return db["processing_stats"]
...
def get_tts_user_voice(user_id: str) -> str:
    """Return TTS voice from MongoDB, or default."""
    settings = get_tts_settings_collection().find_one({"_id": user_id})
...
def set_tts_user_voice(user_id: str, voice: str):
    ...
    get_tts_settings_collection().update_one(
        {"_id": user_id},
        {"$set": {"voice": voice}},
        upsert=True
    )
...
def get_tts_user_pitch(user_id: str) -> int:
    """Return TTS pitch from MongoDB, or default."""
    settings = get_tts_settings_collection().find_one({"_id": user_id})
...
def get_tts_user_rate(user_id: str) -> int:
    """Return TTS rate from MongoDB, or default."""
    settings = get_tts_settings_collection().find_one({"_id": user_id})
...
def get_stt_user_lang(user_id: str) -> str:
    """Return STT language from MongoDB, or default."""
    settings = get_stt_settings_collection().find_one({"_id": user_id})
...
def set_stt_user_lang(user_id: str, lang_code: str):
    ...
    get_stt_settings_collection().update_one(
        {"_id": user_id},
...
def add_processing_stat(stat: dict):
    ...
    get_processing_stats_collection().insert_one(stat)

# ────────────────────────────────────────────────────────────────────────────────
#   I N - M E M O R Y   H E L P E R   F U N C T I O N S (for child bots only)
# ────────────────────────────────────────────────────────────────────────────────
...
2.  Speech-to-Text (STT) Conversion
     Use /languagestt to **set language**.
...
def total_users(message):
    total_registered = get_users_collection().count_documents({})
...
def broadcast_message(message):
    ...
    # Iterate through users in MongoDB
    for user_doc in get_users_collection().find({}, {"_id": 1}):
        uid = user_doc["_id"]
        if uid == str(ADMIN_ID):
            continue
        try:
...
def handle_stt_media_types_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    if user_id_for_settings not in get_stt_settings_collection().find_one({"_id": user_id_for_settings}) if get_stt_settings_collection().find_one({"_id": user_id_for_settings}) else {}:
...
def set_webhook_on_startup():
    try:
        ...
        # Re-set webhooks for any child bots that were registered in memory
...
def set_bot_info_and_startup():
    global bot_start_time
    bot_start_time = datetime.now()
    init_mongo() # Initialize MongoDB connection
