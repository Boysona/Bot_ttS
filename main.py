...
in_memory_data = {
    "users": {},            # { user_id: { "last_active": "...", "tts_conversion_count": N, "stt_conversion_count": N, "is_premium": False, ... } }
...
user_awaiting_admin_action = {} # { admin_id: "awaiting_user_id_for_premium_access" }
user_premium_access_requests = {} # { user_id: True } # Store premium access requests
...
def update_user_activity_in_memory(user_id: int):
    ...
    Also ensures new users are created with default counts and premium status.
    """
...
def set_user_premium_status(user_id: int, is_premium: bool):
    """
    Sets the premium status of a user.
    """
    user_id_str = str(user_id)
    if user_id_str in in_memory_data["users"]:
        in_memory_data["users"][user_id_str]["is_premium"] = is_premium
        logging.info(f"User {user_id} premium status set to {is_premium}")
    else:
        # Create user if they don't exist and set premium
        update_user_activity_in_memory(user_id)
        in_memory_data["users"][user_id_str]["is_premium"] = is_premium
        logging.info(f"New user {user_id} created and premium status set to {is_premium}")

def is_user_premium(user_id: int) -> bool:
    """
    Checks if a user is premium.
    """
    user_data = get_user_data_in_memory(str(user_id))
...
def start_handler(message):
    ...
    user_awaiting_admin_action[message.from_user.id] = None # Clear admin awaiting state
    
    # Admin state is specifically for admin broadcast, not general user modes
    if message.from_user.id == ADMIN_ID:
        keyboard = telebot.types.ReplyKeyboardMarkup(resize_keyboard=True)
        keyboard.add("Send Broadcast", "Total Users", "/status", "Accept User")
...
@bot.message_handler(func=lambda m: m.text == "Accept User" and m.from_user.id == ADMIN_ID)
def accept_user_prompt(message):
    user_awaiting_admin_action[message.from_user.id] = "awaiting_user_id_for_premium_access"
    bot.send_message(message.chat.id, "Please send the User ID of the user you want to grant premium access to (for /register_bot).")

@bot.message_handler(func=lambda m: m.from_user.id == ADMIN_ID and user_awaiting_admin_action.get(m.from_user.id) == "awaiting_user_id_for_premium_access")
def grant_user_premium_access(message):
    admin_id = message.from_user.id
    user_awaiting_admin_action[admin_id] = None # Clear admin state

    try:
        target_user_id = int(message.text.strip())
        set_user_premium_status(target_user_id, True)
        bot.send_message(admin_id, f"Success! User `{target_user_id}` has been granted premium access for /register_bot.", parse_mode="Markdown")
        # Notify the user if they exist and are not the admin
        if str(target_user_id) in in_memory_data["users"] and target_user_id != ADMIN_ID:
            try:
                bot.send_message(target_user_id, "🎉 Good news! You've been granted premium access and can now use the /register_bot command to create your own dedicated bots!")
            except telebot.apihelper.ApiTelegramException as e:
                logging.warning(f"Could not notify user {target_user_id} about premium access: {e}")
                bot.send_message(admin_id, f"Could not notify user {target_user_id} directly, but their premium status is updated.")
        # Remove from premium access requests if it was there
        if target_user_id in user_premium_access_requests:
            del user_premium_access_requests[target_user_id]

    except ValueError:
        bot.send_message(admin_id, "Invalid User ID. Please send a valid integer User ID.")
    except Exception as e:
        logging.error(f"Error granting premium access: {e}")
        bot.send_message(admin_id, f"An error occurred while granting access: {e}")
...
def register_bot_command(message):
    ...
    if not is_user_premium(message.from_user.id):
        markup = InlineKeyboardMarkup()
        markup.add(InlineKeyboardButton("Contact Developer", url="https://t.me/user33230"))
        bot.send_message(
            message.chat.id,
            "The /register_bot command is a **premium feature**.\n\n"
            "If you'd like to create your own dedicated bot without coding or hosting, "
            "please contact the developer:",
            reply_markup=markup,
            parse_mode="Markdown"
        )
        # Optionally, log the request if you want to track who tried to access premium feature
        user_premium_access_requests[message.from_user.id] = True 
        logging.info(f"User {message.from_user.id} attempted to use /register_bot (premium feature).")
        return
...
async def synth_and_send_tts(chat_id: int, user_id_for_settings: str, text: str, target_bot: telebot.TeleBot):
    ...
    filename = f"tts_audio_cache/tts_{user_id_for_settings}_{uuid.uuid4()}.mp3" # Save to cache directory
    ...
    try:
        ...
        if res['status'] == 'completed':
            ...
            if not text:
                ...
                target_bot.send_document(chat_id, f) # Send as document without reply
...
def handle_text_for_tts_or_mode_input_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    if current_voice:
        if len(message.text) > 4000: # Increased character limit
            target_bot.send_message(message.chat.id, "Please keep your text under 4,000 characters for text-to-speech conversion.")
            return
...
def child_webhook(child_bot_token):
    if request.method == "POST":
        ...
        if content_type and content_type.startswith("application/json"):
            ...
            user_first_name = ""
            if message and message.from_user.first_name:
                user_first_name = message.from_user.first_name
            elif callback_query and callback_query.from_user.first_name:
                ...
                if message.text and message.text.startswith('/start'):
                    if service_type == "tts":
                        welcome_message = (
                            ...
                            f"\t• Send me any text and I’ll convert it to audio,\n"
                            f"\t• Then send it back to you!\n"
                            f"\t• Choose your text language and avatar speaking type,\n"
                            f"\t• Or click /voice\n"
                            f"\t• For more commands, go to the Menu"
                        )
                    elif service_type == "stt":
                        welcome_message = (
                            f"👋Salam {user_first_name}\n"
                            f"\t• Send a voice, video, or audio file,\n"
                            f"\t• I’ll transcribe it and send it back to you!\n"
                            f"\t• Choose your media file language,\n"
                            f"\t• Or click /language_stt"
                        )
