...
# --- BOT CONFIGURATION ---
TOKEN = "7560825699:AAEzxr49ELI64Ndfy1hVMMcX6-DTV2CqXGA"  # <-- Main Bot Token
ADMIN_ID = 5978150981
...
def broadcast_message(message):
    ...
    for uid in in_memory_data["users"].keys():
        if uid == str(ADMIN_ID):
            continue
        try:
            bot.copy_message(uid, message.chat.id, message.message.id)
            success += 1
        except telebot.apihelper.ApiTelegramException as e:
...
async def process_stt_media(chat_id: int, user_id_for_settings: str, message_type: str, file_id: str, target_bot: telebot.TeleBot, original_message_id: int):
    ...
    `target_bot` is the bot instance to use for sending messages (main or child bot).
    `user_id_for_settings` is the actual user's ID for settings lookup.
    `original_message_id` is the ID of the message to which the "Processing..." reply should be sent.
    """
    ...
    processing_msg = None
    try:
        # Send "Processing..." as a reply to the original message
        processing_msg = target_bot.send_message(chat_id, "⌛️ Processing…", reply_to_message_id=original_message_id)
        ...
        if res['status'] == 'completed':
            ...
            if not text:
                ...
                target_bot.send_message(chat_id, text, reply_to_message_id=original_message_id) # Reply with transcription
            else:
                import io
                f = io.BytesIO(text.encode("utf-8"))
                f.name = "transcript.txt"
                target_bot.send_document(chat_id, f, caption="Your transcription is too long for a single message. Here's the text file:", reply_to_message_id=original_message_id) # Reply with document
            ...
            target_bot.send_message(chat_id, f"❌ Transcription error: `{error_msg}`", parse_mode="Markdown", reply_to_message_id=original_message_id) # Reply with error
        ...
        target_bot.send_message(chat_id, "❌ A network error occurred while processing your file. Please try again.", reply_to_message_id=original_message_id) # Reply with error
        ...
        target_bot.send_message(chat_id, "❌ Oops! An unexpected error occurred during transcription. Please try again in a moment.", reply_to_message_id=original_message_id) # Reply with error
        status = "fail_unknown"
        processing_time = (datetime.now() - processing_start_time).total_seconds() if 'processing_start_time' in locals() else 0
        add_processing_stat_in_memory({
            "user_id": user_id_for_settings, # Use the actual user's ID for stats
...
def handle_text_for_tts_or_mode_input_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    if current_voice:
        # UPDATED: Character limit for TTS conversion
        if len(message.text) > 4000:
            target_bot.send_message(message.chat.id, "Please keep your text under 4,000 characters for text-to-speech conversion to ensure optimal performance.")
            return
