...
# --- BOT CONFIGURATION ---
TOKEN = "7560825699:AAEzxr49ELI64Ndf1hVMMcX6-DTV7CqXGA"  # <-- Main Bot Token
ADMIN_ID = 5978150981
...
If you have any questions or run into any issues, feel free to reach out to @user33230.
    ...
    try:
        ...
        if res['status'] == 'completed':
            ...
            if not text:
                ...
                # For document replies, reply_to_message_id is also appropriate
        ...
        target_bot.send_message(chat_id, "❌ Oops! An unexpected error occurred during transcription. Please try again in a moment.", reply_to_message_id=original_message_id)
        status = "fail_unknown"
        processing_time = (datetime.now() - processing_start_time).total_seconds() if 'processing_start_time' in locals() else 0
        add_processing_stat_in_memory({
            "user_id": user_id_for_settings, # Use the actual user's ID for stats
...
def handle_text_for_tts_or_mode_input_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    if current_voice:
        # Removed the 1000 character limit for TTS.
        # Max message length for Telegram is 4096 characters, but msspeech might have its own limits.
        # For simplicity, we'll allow up to 4000 characters for now as per the user's request,
        # and let msspeech handle potential longer text limitations internally or throw an error.
        if len(message.text) > 4000: # Telegram message limit for text
             target_bot.send_message(message.chat.id, "Your text is too long. Please keep it under 4000 characters for text-to-speech conversion.")
             return
        ...
        if content_type and content_type.startswith("application/json"):
            ...
            if message:
                ...
                if message.text and message.text.startswith('/start'):
                    user_first_name = message.from_user.first_name if message.from_user.first_name else ""
                    if service_type == "tts":
                        welcome_message = (
                            f"Salaam {user_first_name}\n\n"
                            "• Send me any text, I will convert it to audio and send it back to you!\n"
                            "• Choose your text language and avatar speaking type or click /voice\n"
                            "• For more commands Menu"
                        )
                        ...
                        welcome_message = (
                            ...
                            "• Choose your media file language type or click /language_stt"
                        )
