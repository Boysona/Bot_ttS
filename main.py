...
def start_handler(message):
    ...
    if message.from_user.id == ADMIN_ID:
        # No initial message for admin on /start
        pass
    else:
        welcome_message = (
...
def set_bot_commands():
    ...
    # Add /access command only for admin
    admin_commands = [
        BotCommand("access", "Access Admin Panel")
    ]
    try:
        bot.set_my_commands(commands)
        # Set specific commands for admin scope (requires Telegram Bot API 5.0+)
        bot.set_my_commands(admin_commands, scope=telebot.types.BotCommandScopeChat(chat_id=ADMIN_ID))
