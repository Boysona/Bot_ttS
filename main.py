...
db = None
users_collection = None # Stores general user data like last_active, conversion counts
tts_users_collection = None # Stores TTS specific user settings (voice, pitch, rate)
processing_stats_collection = None
stt_users_collection = None # Stores STT specific user settings (language)
registered_bots_collection = None # To manage child bots

# --- In-memory caches ---
# Only keeping local_user_data for last_active and conversion counts
# User-specific settings like voice, pitch, rate, and STT language will be fetched directly from MongoDB
...
# These states are temporary and specific to the current interaction, not persistent settings
...
user_register_bot_mode = {}     # { owner_id: "awaiting_token" or "awaiting_service_type" }
...
def connect_to_mongodb():
    ...
    Also, load registered bot data into in-memory cache.
    """
    ...
    global local_user_data, _registered_bots_cache
    ...
    try:
        ...
        # No need for indexes on _id for tts_users_collection and stt_users_collection as _id is already indexed by default
        ...
        logging.info("Connected to MongoDB and indexes created. Loading essential data to memory...")

        # --- Load only general user data and registered bots into in-memory caches on startup ---
        # User settings (voice, pitch, rate, STT language) are fetched on demand from DB now.
        for user_doc in users_collection.find({}):
            local_user_data[user_doc["_id"]] = user_doc
        logging.info(f"Loaded {len(local_user_data)} user documents into local_user_data (for activity/counts).")
        ...
        logging.info("Essential data loaded into in-memory caches.")
...
def update_user_activity_db(user_id: int):
    ...
    # Update in-memory cache for general user data
...
def increment_processing_count_db(user_id: str, service_type: str):
    ...
    # Update in-memory cache for general user data (counts and last_active)
    if user_id_str not in local_user_data:
        local_user_data[user_id_str] = {
            "_id": user_id_str,
            "last_active": now_iso,
...
def get_tts_user_voice_db(user_id: str) -> str:
    """
    Return TTS voice from MongoDB for a specific user. Default to "so-SO-MuuseNeural".
    """
    try:
        doc = tts_users_collection.find_one({"_id": user_id})
        return doc.get("voice", "so-SO-MuuseNeural") if doc else "so-SO-MuuseNeural"
    except Exception as e:
        logging.error(f"Error fetching TTS voice for {user_id} from DB: {e}")
        return "so-SO-MuuseNeural" # Fallback default

def set_tts_user_voice_db(user_id: str, voice: str):
    """
    Save TTS voice in DB for a specific user.
    """
    try:
...
def get_tts_user_pitch_db(user_id: str) -> int:
    """
    Return TTS pitch from MongoDB for a specific user. Default to 0.
    """
    try:
        doc = tts_users_collection.find_one({"_id": user_id})
        return doc.get("pitch", 0) if doc else 0
    except Exception as e:
        logging.error(f"Error fetching TTS pitch for {user_id} from DB: {e}")
        return 0 # Fallback default

def set_tts_user_pitch_db(user_id: str, pitch: int):
    """
    Save TTS pitch in DB for a specific user.
    """
    try:
        tts_users_collection.update_one(
            {"_id": user_id},
...
def get_tts_user_rate_db(user_id: str) -> int:
    """
    Return TTS rate from MongoDB for a specific user. Default to 0.
    """
    try:
        doc = tts_users_collection.find_one({"_id": user_id})
        return doc.get("rate", 0) if doc else 0
    except Exception as e:
        logging.error(f"Error fetching TTS rate for {user_id} from DB: {e}")
        return 0 # Fallback default

def set_tts_user_rate_db(user_id: str, rate: int):
    """
    Save TTS rate in DB for a specific user.
    """
    try:
        tts_users_collection.update_one(
            {"_id": user_id},
...
def get_stt_user_lang_db(user_id: str) -> str:
    """
    Return STT language from MongoDB for a specific user. Default to "en".
    """
    try:
        doc = stt_users_collection.find_one({"_id": user_id})
        return doc.get("language_code", "en") if doc else "en"
    except Exception as e:
        logging.error(f"Error fetching STT language for {user_id} from DB: {e}")
        return "en" # Fallback default

def set_stt_user_lang_db(user_id: str, lang_code: str):
    """
    Save STT language in DB for a specific user.
    """
    try:
...
def on_register_bot_service_select(call):
    ...
    if register_child_bot_db(bot_token, uid, service_type):
        try:
            ...
            # Consider rolling back DB entry if webhook fails critically
        except Exception as e:
...
def handle_rate_command(message, target_bot: telebot.TeleBot, owner_id: str):
    ...
    chat_id = message.chat.id
    # Use owner_id for state management
    user_id_for_state = owner_id

    # Clear other modes for this owner_id
    user_tts_mode[user_id_for_state] = None
    user_pitch_input_mode[user_id_for_state] = None
    user_rate_input_mode[user_id_for_state] = "awaiting_rate_input" # Set this mode
    user_register_bot_mode[user_id_for_state] = None # Clear this mode
...
def handle_pitch_command(message, target_bot: telebot.TeleBot, owner_id: str):
    ...
    # Clear other modes for this owner_id
    user_tts_mode[user_id_for_state] = None
    user_pitch_input_mode[user_id_for_state] = "awaiting_pitch_input" # Set this mode
    user_rate_input_mode[user_id_for_state] = None
    user_register_bot_mode[user_id_for_state] = None # Clear this mode
...
def handle_pitch_callback(call, target_bot: telebot.TeleBot, owner_id: str):
    ...
    user_pitch_input_mode[user_id_for_state] = None # Clear mode
...
def handle_tts_voice_change_callback(call, target_bot: telebot.TeleBot, owner_id: str):
    ...
    user_tts_mode[user_id_for_state] = voice
...
def handle_stt_language_select_callback(call, target_bot: telebot.TeleBot, owner_id: str):
    ...
    set_stt_user_lang_db(user_id_for_state, lang_code)
...
async def process_stt_media(chat_id: int, user_id: str, message_type: str, file_id: str, target_bot: telebot.TeleBot):
    ...
    try:
        ...
        lang_code = get_stt_user_lang_db(user_id) # Get STT language preference from DB
...
def handle_stt_media_types_common(message, target_bot: telebot.TeleBot, owner_id: str):
    ...
    user_register_bot_mode[owner_id] = None # Clear this mode
    ...
    # This will now fetch directly from DB. If not found, get_stt_user_lang_db returns default "en".
    # We can rely on the default and don't need a separate check here.
...
def handle_text_for_tts_or_mode_input_common(message, target_bot: telebot.TeleBot, owner_id: str):
    ...
    current_voice = get_tts_user_voice_db(owner_id) # Fetch current voice from DB
                ...
                if message.text and message.text.startswith('/start'):
                    ...
                    if service_type == "tts":
                        ...
                        welcome_message += (
                            ...
                            "Send me a voice message, audio, or video file to transcribe! Use `/language_stt` to change the language."
                        )
...
def set_webhook_on_startup():
    try:
        ...
        # Also, set webhooks for all previously registered child bots on startup
        for token, info in _registered_bots_cache.items():
