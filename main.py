...
from pymongo.errors import ConnectionFailure, DuplicateKeyError # Import DuplicateKeyError
...
# --- BOT CONFIGURATION ---
TOKEN = "8089409453:AAH-DjTRVY1H5kMp_ymE6ThYAl4zC4BGTwM"  # <-- Main Bot Token
ADMIN_ID = 5978150981
...
_registered_bots_cache = {}     # { composite_key: { "token": "...", "owner_id": "...", "service_type": ... } }
...
def connect_to_mongodb():
    ...
    try:
        ...
        # The issue might stem from this index if a bot with the same token is attempted to be registered
        # under a different composite_key.
        # It's better to make _id unique, and if you want token to be unique, ensure that logic handles it.
        # For simplicity, if _id (composite_key) is unique, and you want to prevent duplicate tokens,
        # you need to check for existing tokens *before* attempting insertion/upsert.
        registered_bots_collection.create_index([("token", ASCENDING)], unique=True) # Ensure this index exists
        registered_bots_collection.create_index([("_id", ASCENDING)], unique=True) # Make sure _id is also unique
        ...
        for tts_user in tts_users_collection.find({}):
            # Ensure _id is treated as string for consistency with cache keys
            _tts_voice_cache[str(tts_user["_id"])] = tts_user.get("voice", "so-SO-MuuseNeural") # Default to Somali for TTS
        ...
        for stt_user in stt_users_collection.find({}): # Load STT user settings
            # Ensure _id is treated as string for consistency with cache keys
            _stt_lang_cache[str(stt_user["_id"])] = stt_user.get("language_code", "en") # Default to English for STT
        logging.info(f"Loaded {len(_stt_lang_cache)} STT language settings.")

        for bot_doc in registered_bots_collection.find({}):
            composite_key = f"{bot_doc['owner_id']}_{bot_doc['service_type']}"
            _registered_bots_cache[composite_key] = {
                "token": bot_doc["token"],
...
def register_child_bot_db(token: str, owner_id: str, service_type: str):
    """
    Registers a new child bot in the database using composite key
    """
    composite_key = f"{owner_id}_{service_type}"
    
    try:
        # Check if a bot with this token already exists regardless of owner/service_type
        # This handles the unique index on 'token'
        if registered_bots_collection.find_one({"token": token}):
            logging.warning(f"Attempted to register duplicate token: {token[:5]}... for owner {owner_id}, service {service_type}")
            # You might want to return a specific error code or message here
            # to differentiate between a general DB error and a duplicate token.
            return False # Indicate failure due to duplicate token
        ...
        logging.info(f"Child bot registered for owner {owner_id} with service {service_type}")
        return True
    except DuplicateKeyError as e:
        logging.error(f"MongoDB DuplicateKeyError during child bot registration for owner {owner_id}, service {service_type}, token {token[:5]}...: {e}")
        # This specifically catches if _id (composite_key) is duplicated, which the previous check already handled.
        # However, it's good to keep it separate if you decide to remove the token check above.
        return False
    except ConnectionFailure as e:
        logging.error(f"MongoDB connection failed during child bot registration: {e}")
        return False
    except Exception as e:
        logging.error(f"Error registering child bot for owner {owner_id}: {e}")
        return False

def get_child_bot_info_by_composite_key(composite_key: str) -> dict | None:
    """
    Retrieves child bot information using composite key
    """
    if composite_key in _registered_bots_cache:
        return _registered_bots_cache[composite_key]
    
    try:
        doc = registered_bots_collection.find_one({"_id": composite_key})
        if doc:
            _registered_bots_cache[composite_key] = {
                "token": doc["token"],
        ...
        logging.error(f"Error fetching child bot info for {composite_key}: {e}")
        return None
...
def on_register_bot_service_select(call):
    ...
    # Check if this user already has a bot for this service type
    composite_key = f"{uid}_{service_type}"
    if composite_key in _registered_bots_cache:
        try:
            existing_token = _registered_bots_cache[composite_key]["token"]
            temp_bot = telebot.TeleBot(existing_token)
            bot_name = temp_bot.get_me().username
            bot_url = f"https://t.me/{bot_name}"
        except:
            bot_url = "your existing bot"
            
        bot.answer_callback_query(call.id, "You already have a bot for this service!")
        bot.edit_message_text(
            chat_id=call.message.chat.id,
            message_id=call.message.message_id,
            text=f"❌ You already have a {service_type.upper()} bot registered!\n\n"
                 f"You can access it here: {bot_url}\n\n"
                 f"To create a new bot, first revoke the old token in @BotFather.",
            parse_mode="Markdown"
        )
        return

    # Check if the token is already registered by *any* user
    # This check is crucial given the unique index on 'token'
    if registered_bots_collection.find_one({"token": bot_token}):
        bot.answer_callback_query(call.id, "This bot token is already registered by another user or for a different service.")
        bot.edit_message_text(
            chat_id=call.message.chat.id,
            message_id=call.message.message_id,
            text=f"❌ This bot token has already been registered! "
                 "If you own this bot, please revoke the token in @BotFather and try with a new token.",
            parse_mode="Markdown"
        )
        user_register_bot_mode[uid] = None # Clear state
        return
    ...
    if register_child_bot_db(bot_token, uid, service_type):
        try:
            ...
            composite_key = f"{uid}_{service_type}"
            child_bot_webhook_url = f"{WEBHOOK_URL}child_webhook/{composite_key}"
        ...
        # This branch is hit if register_child_bot_db returns False.
        # The specific message "Database error" might be too generic here if it's a duplicate token.
        # Let's adjust the message based on the more specific check added above.
        bot.answer_callback_query(call.id, "Failed to register your bot. Please try again.")
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                              text="Failed to register your bot. A database error occurred or the token is already in use. Please try again later or use a different token.")
...
async def process_stt_media(chat_id: int, user_id_for_settings: str, message_type: str, file_id: str, target_bot: telebot.TeleBot):
    """
...
def handle_stt_media_types_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    """
    update_user_activity_db(int(user_id_for_settings)) # Update this user's activity
    ...
    if user_id_for_settings not in _stt_lang_cache:
        target_bot.send_message(message.chat.id, "❗ Please choose a language for transcription first using /language_stt.")
        return

    threading.Thread(
        target=lambda: asyncio.run(process_stt_media(message.chat.id, user_id_for_settings, message_type, file_id, target_bot))
    ).start()
...
def handle_text_for_tts_or_mode_input_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    `user_id_for_settings` is the actual user's ID for settings lookup and activity updates.
    """
    update_user_activity_db(int(user_id_for_settings))
    ...
    if current_voice:
        # FIXED: Changed character limit from 1000 to 4000
        if len(message.text) > 4000:
            target_bot.send_message(message.chat.id, "Please keep your text under 4000 characters for text-to-speech conversion.")
            return
...
@app.route("/child_webhook/<composite_key>", methods=["POST"])
def child_webhook(composite_key):
    if request.method == "POST":
        content_type = request.headers.get("Content-Type", "")
        if content_type and content_type.startswith("application/json"):
            update = telebot.types.Update.de_json(request.get_data().decode("utf-8"))

            bot_info = get_child_bot_info_by_composite_key(composite_key)
            if not bot_info:
                logging.warning(f"Received update for unregistered composite key: {composite_key}")
            ...
            child_bot_instance = telebot.TeleBot(bot_token, threaded=True)
            ...
            if not user_id_for_settings:
                logging.warning(f"Could not determine user_id for child bot update for composite key: {composite_key}")
            ...
            if message:
                ...
                if message.text and message.text.startswith('/start'):
                    ...
                    if service_type == "tts":
                        current_voice = get_tts_user_voice_db(user_id_for_settings) # Use user_id_for_settings
                        current_pitch = get_tts_user_pitch_db(user_id_for_settings) # Use user_id_for_settings
                        current_rate = get_tts_user_rate_db(user_id_for_settings)   # Use user_id_for_settings
                        welcome_message += (
                        ...
                        current_lang_code = get_stt_user_lang_db(user_id_for_settings) # Use user_id_for_settings
                        current_lang_name = next((name for name, code in STT_LANGUAGES.items() if code == current_lang_code), "English")
                        welcome_message += (
                            f"I transcribe your speech and audio to text. Your current transcription language (managed through the main bot or here): *{current_lang_name}*.\n\n"
...
def set_webhook_on_startup():
    try:
        ...
        for composite_key, info in _registered_bots_cache.items():
            child_bot_webhook_url = f"{WEBHOOK_URL}child_webhook/{composite_key}"
            try:
                child_bot_instance = telebot.TeleBot(info["token"])
                ...
                logging.info(f"Webhook re-set for child bot {composite_key} to {child_bot_webhook_url}")
            except telebot.apihelper.ApiTelegramException as e:
                logging.error(f"Failed to re-set webhook for child bot {composite_key} on startup: {e}")
            except Exception as e:
                logging.error(f"Unexpected error re-setting webhook for child bot {composite_key} on startup: {e}")
