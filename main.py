...
in_memory_data = {
    ...
    "stt_settings": {},
    "registered_bots": {},
    # "processing_stats": [] # Removed this section entirely
}
...
def init_in_memory_data():
    ...
    # No need to initialize processing_stats if it's removed
...
# Removed the add_processing_stat_in_memory function
...
# Removed the entire update_uptime_message function
...
def start_handler(message):
    ...
    if message.from_user.id == ADMIN_ID:
        # Replaced ReplyKeyboardMarkup with InlineKeyboardMarkup
        admin_markup = InlineKeyboardMarkup()
        admin_markup.add(
            InlineKeyboardButton("Send Broadcast", callback_data="admin_broadcast"),
            InlineKeyboardButton("Total Users", callback_data="admin_total_users")
        )
        # Removed the uptime message and the associated thread
        bot.send_message(
            message.chat.id,
...
# Replaced the message handlers with a new callback handler for admin menu
...
async def synth_and_send_tts(chat_id: int, user_id_for_settings: str, text: str, target_bot: telebot.TeleBot):
    ...
    try:
        ...
        with open(filename, "rb") as f:
            # Updated the caption with the new message and default voice info
            caption_text = (
                f"🎧 *Here is your audio!* \n\n"
                f"Voice: en-US-ChristopherNeural\n"
                f"Pitch: 0\n"
                f"Rate: 0\n\n"
                f"Enjoy listening! ✨ \n\n"
                f"Your default voice style is **en-US-ChristopherNeural**. "
                f"You can choose a different voice at any time by using the `/voice` command."
            )

            # Check if the user has already set a voice to decide the caption text
            if get_tts_user_voice_in_memory(user_id_for_settings) != "so-SO-MuuseNeural":
                caption_text = (
                    f"🎧 Here is your audio!\n\n"
                    f"Voice: *{voice}*\n"
                    f"Pitch: *{pitch}*\n"
        ...
        # Removed: increment_processing_count_in_memory(user_id_for_settings, "tts")
        # Removed: add_processing_stat_in_memory(...)
