...
tts_settings_collection = None
stt_settings_collection = None
# registered_bots_collection = None # This will now be in-memory
processing_stats_collection = None

# --- In-memory storage for registered child bots ---
# Key: bot_token, Value: {"owner_id": str, "service_type": str, "registration_date": datetime}
# This data will be lost if the application restarts.
registered_bots_in_memory = {}

def connect_to_mongodb():
    """
    Establishes connection to MongoDB and sets up global collection objects.
    """
    global mongo_client, db, users_collection, tts_settings_collection, stt_settings_collection, processing_stats_collection
    try:
        ...
        # registered_bots_collection = db["registered_bots"] # Removed as it's now in-memory
        processing_stats_collection = db["processing_stats"]
        logging.info("Successfully connected to MongoDB.")
    except ConnectionFailure as e:
        logging.error(f"MongoDB connection failed: {e}")
        # Exit or handle gracefully, bot won't function without DB
        exit(1)
...
# Admin uptime message storage (still in-memory as it's volatile)
...
def update_user_activity(user_id: int):
    ...
    try:
        ...
        logging.error(f"MongoDB operation failed for update_user_activity: {e}")
...
def get_user_data(user_id: str) -> dict | None:
    ...
    try:
        ...
        logging.error(f"MongoDB operation failed for get_user_data: {e}")
        return None
...
def increment_processing_count(user_id: str, service_type: str):
    ...
    try:
        users_collection.update_one(
            {"_id": user_id_str},
            {"$inc": {field_to_inc: 1}, "$set": {"last_active": now_iso}},
            upsert=True # In case the user doc somehow doesn't exist yet
        )
    except OperationFailure as e:
        logging.error(f"MongoDB operation failed for increment_processing_count: {e}")
...
def get_tts_user_voice(user_id: str) -> str:
    ...
    try:
        ...
        logging.error(f"MongoDB operation failed for get_tts_user_voice: {e}")
...
def set_tts_user_voice(user_id: str, voice: str):
    ...
    try:
        ...
        logging.error(f"MongoDB operation failed for set_tts_user_voice: {e}")
...
def get_tts_user_pitch(user_id: str) -> int:
    ...
    try:
        ...
        logging.error(f"MongoDB operation failed for get_tts_user_pitch: {e}")
        return 0
...
def set_tts_user_pitch(user_id: str, pitch: int):
    ...
    try:
        ...
        logging.error(f"MongoDB operation failed for set_tts_user_pitch: {e}")
...
def get_tts_user_rate(user_id: str) -> int:
    ...
    try:
        ...
        logging.error(f"MongoDB operation failed for get_tts_user_rate: {e}")
        return 0
...
def set_tts_user_rate(user_id: str, rate: int):
    ...
    try:
        ...
        logging.error(f"MongoDB operation failed for set_tts_user_rate: {e}")
...
def get_stt_user_lang(user_id: str) -> str:
    ...
    try:
        ...
        logging.error(f"MongoDB operation failed for get_stt_user_lang: {e}")
        return "en"
...
def set_stt_user_lang(user_id: str, lang_code: str):
    ...
    try:
        ...
        logging.error(f"MongoDB operation failed for set_stt_user_lang: {e}")

def register_child_bot(token: str, owner_id: str, service_type: str):
    """
    Registers a new child bot in in-memory storage, or updates it if already registered.
    Returns True on success, False if an unexpected error occurs during registration.
    """
    try:
        # Check if already registered in memory
        existing_bot = registered_bots_in_memory.get(token)
        if existing_bot:
            # If already registered, update its owner and service type if needed.
            # No need to update registration_date as it implies initial registration.
            if existing_bot.get("owner_id") != owner_id or existing_bot.get("service_type") != service_type:
                registered_bots_in_memory[token].update({
                    "owner_id": owner_id,
                    "service_type": service_type
                })
                logging.info(f"Child bot {token[:5]}... updated for owner {owner_id} with service {service_type} in memory.")
            else:
                logging.info(f"Child bot {token[:5]}... already registered and up-to-date for owner {owner_id} with service {service_type} in memory.")
            return True # Token already exists and is considered "OK"

        # If not already registered, insert new into memory
        registered_bots_in_memory[token] = {
            "owner_id": owner_id,
            "service_type": service_type,
            "registration_date": datetime.now()
        }
        logging.info(f"Child bot {token[:5]}... newly registered for owner {owner_id} with service {service_type} in memory.")
        return True
    except Exception as e:
        logging.error(f"Error registering child bot in memory: {e}")
        return False # Only return False if there's an in-memory storage error

def get_child_bot_info(token: str) -> dict | None:
    """
    Retrieves child bot information from in-memory storage.
    """
    try:
        return registered_bots_in_memory.get(token)
    except Exception as e:
        logging.error(f"Error retrieving child bot info from memory: {e}")
        return None
...
def add_processing_stat(stat: dict):
    ...
    try:
        ...
        logging.error(f"MongoDB operation failed for add_processing_stat: {e}")
...
1.  **Text-to-Speech (TTS) Conversion**
    * Use /voice to **choose a voice**, then send text.
    * Use /pitch to **adjust tone**.
    * Use /rate to **change speed**.

2.  **Speech-to-Text (STT) Conversion**
    * Use /language_stt to **set language**.
    * Send a voice, audio, or video file (max 20MB).

3.  **Create Your Own Bot!**
    * Use /register_bot to make a dedicated TTS or STT bot.

4.  **Privacy:** Your content is processed instantly and **never stored**. Settings are saved to make your experience seamless.

---
If you need help, contact @user33230.
Enjoy! ✨
"""
    )
...
# Removed the /status handler entirely as per the request.
...
def total_users(message):
    try:
        ...
        logging.error(f"MongoDB operation failed for total_users: {e}")
        bot.send_message(message.chat.id, "Failed to retrieve total users from database.")
...
def broadcast_message(message):
    ...
    success = fail = 0
    try:
        for user_doc in users_collection.find({}, {"_id": 1}): # Fetch only _id
            uid = user_doc["_id"]
            if uid == str(ADMIN_ID):
                continue
            try:
        ...
        logging.error(f"MongoDB operation failed during broadcast: {e}")
        bot.send_message(message.chat.id, "Failed to retrieve user list for broadcast from database.")
...
def on_register_bot_service_select(call):
    ...
    # Call the modified register_child_bot function (now saving to memory)
    if register_child_bot(bot_token, uid, service_type):
        ...
        # This branch will now only be hit if there's an actual in-memory storage error (unlikely for a simple dict)
        bot.answer_callback_query(call.id, "Failed to register your bot due to an internal error. Please try again.")
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                              text="Failed to register your bot due to an internal error. Please try again later.")
...
async def synth_and_send_tts(chat_id: int, user_id_for_settings: str, text: str, target_bot: telebot.TeleBot):
    ...
    try:
        ...
        increment_processing_count(user_id_for_settings, "tts") # Increment user's TTS count
...
async def process_stt_media(chat_id: int, user_id_for_settings: str, message_type: str, file_id: str, target_bot: telebot.TeleBot, original_message_id: int): # Added original_message_id
    ...
    try:
        ...
        if res['status'] == 'completed':
            ...
            increment_processing_count(user_id_for_settings, "stt") # Increment user's STT count
            status = "success"
        else:
...
def handle_stt_media_types_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    """
    update_user_activity(int(user_id_for_settings)) # Update this user's activity
    ...
    # This checks against MongoDB, not in-memory
...
def child_webhook(child_bot_token):
    if request.method == "POST":
        ...
        if content_type and content_type.startswith("application/json"):
            ...
            # Retrieve from in-memory storage instead of MongoDB
...
def set_webhook_on_startup():
    try:
        ...
        # On startup, we can no longer load previously registered child bots from MongoDB.
        # They will only be re-registered if the user goes through the /register_bot process again.
        logging.warning("Child bot registrations are now in-memory and will not persist across restarts.")
...
def set_bot_info_and_startup():
    ...
    set_webhook_on_startup() # This now handles main bot webhooks only for new registrations
