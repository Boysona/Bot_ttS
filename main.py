...
Please make sure that when the bot sends a transcription text, it doesn’t send it as a “reply to result” unless it’s a text file—just like shown in the images.

Also, the welcome message of the child bots should be like this:

⸻

👋Salam (user first name)
...
Also, remove the 1,000 character limit on text-to-speech conversions so that the bot can convert as much as Microsoft’s speech service (msspeech) allows in one go. It shouldn’t be limited to 1,000 characters in the chat—it should support up to 4,000 characters.

⸻

Additionally, the /register_bot command should be for premium users only.

If a regular user tries to use /register_bot, show them a message telling them that if they want to create a bot like this (without coding or hosting), they should contact the developer of the bot: @user33230.

Now, add a new section in the admin panel called “Accept User” button.

When the admin clicks this button, they should be asked to send the ID of the user who is requesting access to the /register_bot command. Once the ID is sent, the admin will be notified that the process was successful, and that user will now be allowed to use the command.
            ...
            if message:
                ...
                if message.text and message.text.startswith('/start'):
                    ...
                    if service_type == "tts":
                        ...
                        current_lang_code = get_stt_user_lang_in_memory(user_id_for_settings) # Use user_id_for_settings
