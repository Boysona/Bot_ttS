...
# --- In-memory data storage (for child bots only, as per requirements) ---
# This will hold all your child bot data in memory.
...
def connect_to_mongodb():
    """
    Establishes connection to MongoDB and sets global client and db objects.
    Returns False on connection error, True otherwise.
    """
...
def update_user_activity(user_id: int):
    """
    Update user.last_active = now() in MongoDB.
    Also ensures new users are created with default counts.
    Returns False on MongoDB error, True otherwise.
    """
    if not db:
        logging.error("MongoDB not connected. Cannot update user activity.")
        return False
    ...
    try:
        ...
        logging.error(f"MongoDB operation error updating user activity for {user_id_str}: {e}")
        return False
    except Exception as e:
        logging.error(f"An unexpected error occurred updating user activity for {user_id_str}: {e}")
        return False

def get_user_data(user_id: str) -> dict | None:
    """
    Return user document from MongoDB. Returns None on MongoDB error.
    """
    if not db:
        logging.error("MongoDB not connected. Cannot get user data.")
        return None
    try:
        return db.users.find_one({"_id": user_id})
    except OperationFailure as e:
        logging.error(f"MongoDB operation error getting user data for {user_id}: {e}")
        return None
    except Exception as e:
        logging.error(f"An unexpected error occurred getting user data for {user_id}: {e}")
        return None

def increment_processing_count(user_id: str, service_type: str):
    """
    Increment either tts_conversion_count or stt_conversion_count in MongoDB.
    Returns False on MongoDB error, True otherwise.
    """
    if not db:
        logging.error("MongoDB not connected. Cannot increment processing count.")
        return False
    ...
    try:
        db.users.update_one(
            ...
            {"$inc": {field_to_inc: 1},
             "$set": {"last_active": now_iso}},
            upsert=True # In case the user was not previously created
        )
        return True
    except OperationFailure as e:
        logging.error(f"MongoDB operation error incrementing count for {user_id_str}: {e}")
        return False
    except Exception as e:
        logging.error(f"An unexpected error occurred incrementing count for {user_id_str}: {e}")
        return False

def get_tts_user_voice(user_id: str) -> str:
    """
    Return TTS voice from MongoDB (default "so-SO-MuuseNeural"). Returns default on MongoDB error.
    """
    if not db:
        logging.error("MongoDB not connected. Cannot get TTS voice. Returning default.")
        return "so-SO-MuuseNeural"
    try:
        user_settings = db.tts_settings.find_one({"_id": user_id})
        return user_settings.get("voice", "so-SO-MuuseNeural") if user_settings else "so-SO-MuuseNeural"
    except OperationFailure as e:
        logging.error(f"MongoDB operation error getting TTS voice for {user_id}: {e}")
        return "so-SO-MuuseNeural"
    except Exception as e:
        logging.error(f"An unexpected error occurred getting TTS voice for {user_id}: {e}")
        return "so-SO-MuuseNeural"

def set_tts_user_voice(user_id: str, voice: str):
    """
    Save TTS voice in MongoDB. Returns False on MongoDB error, True otherwise.
    """
    if not db:
        logging.error("MongoDB not connected. Cannot set TTS voice.")
        return False
    try:
        db.tts_settings.update_one({"_id": user_id}, {"$set": {"voice": voice}}, upsert=True)
        return True
    except OperationFailure as e:
        logging.error(f"MongoDB operation error setting TTS voice for {user_id}: {e}")
        return False
    except Exception as e:
        logging.error(f"An unexpected error occurred setting TTS voice for {user_id}: {e}")
        return False

def get_tts_user_pitch(user_id: str) -> int:
    """
    Return TTS pitch from MongoDB (default 0). Returns default on MongoDB error.
    """
    if not db:
        logging.error("MongoDB not connected. Cannot get TTS pitch. Returning default.")
        return 0
    try:
        user_settings = db.tts_settings.find_one({"_id": user_id})
        return user_settings.get("pitch", 0) if user_settings else 0
    except OperationFailure as e:
        logging.error(f"MongoDB operation error getting TTS pitch for {user_id}: {e}")
        return 0
    except Exception as e:
        logging.error(f"An unexpected error occurred getting TTS pitch for {user_id}: {e}")
        return 0

def set_tts_user_pitch(user_id: str, pitch: int):
    """
    Save TTS pitch in MongoDB. Returns False on MongoDB error, True otherwise.
    """
    if not db:
        logging.error("MongoDB not connected. Cannot set TTS pitch.")
        return False
    try:
        db.tts_settings.update_one({"_id": user_id}, {"$set": {"pitch": pitch}}, upsert=True)
        return True
    except OperationFailure as e:
        logging.error(f"MongoDB operation error setting TTS pitch for {user_id}: {e}")
        return False
    except Exception as e:
        logging.error(f"An unexpected error occurred setting TTS pitch for {user_id}: {e}")
        return False

def get_tts_user_rate(user_id: str) -> int:
    """
    Return TTS rate from MongoDB (default 0). Returns default on MongoDB error.
    """
    if not db:
        logging.error("MongoDB not connected. Cannot get TTS rate. Returning default.")
        return 0
    try:
        user_settings = db.tts_settings.find_one({"_id": user_id})
        return user_settings.get("rate", 0) if user_settings else 0
    except OperationFailure as e:
        logging.error(f"MongoDB operation error getting TTS rate for {user_id}: {e}")
        return 0
    except Exception as e:
        logging.error(f"An unexpected error occurred getting TTS rate for {user_id}: {e}")
        return 0

def set_tts_user_rate(user_id: str, rate: int):
    """
    Save TTS rate in MongoDB. Returns False on MongoDB error, True otherwise.
    """
    if not db:
        logging.error("MongoDB not connected. Cannot set TTS rate.")
        return False
    try:
        db.tts_settings.update_one({"_id": user_id}, {"$set": {"rate": rate}}, upsert=True)
        return True
    except OperationFailure as e:
        logging.error(f"MongoDB operation error setting TTS rate for {user_id}: {e}")
        return False
    except Exception as e:
        logging.error(f"An unexpected error occurred setting TTS rate for {user_id}: {e}")
        return False

def get_stt_user_lang(user_id: str) -> str:
    """
    Return STT language from MongoDB (default "en"). Returns default on MongoDB error.
    """
    if not db:
        logging.error("MongoDB not connected. Cannot get STT language. Returning default.")
        return "en"
    try:
        user_settings = db.stt_settings.find_one({"_id": user_id})
        return user_settings.get("language_code", "en") if user_settings else "en"
    except OperationFailure as e:
        logging.error(f"MongoDB operation error getting STT language for {user_id}: {e}")
        return "en"
    except Exception as e:
        logging.error(f"An unexpected error occurred getting STT language for {user_id}: {e}")
        return "en"

def set_stt_user_lang(user_id: str, lang_code: str):
    """
    Save STT language in MongoDB. Returns False on MongoDB error, True otherwise.
    """
    if not db:
        logging.error("MongoDB not connected. Cannot set STT language.")
        return False
    try:
        db.stt_settings.update_one({"_id": user_id}, {"$set": {"language_code": lang_code}}, upsert=True)
        return True
    except OperationFailure as e:
        logging.error(f"MongoDB operation error setting STT language for {user_id}: {e}")
        return False
    except Exception as e:
        logging.error(f"An unexpected error occurred setting STT language for {user_id}: {e}")
        return False

def add_processing_stat(stat: dict):
    """
    Adds a processing statistic entry to MongoDB. Returns False on MongoDB error, True otherwise.
    """
    if not db:
        logging.error("MongoDB not connected. Cannot add processing stat.")
        return False
    try:
        db.processing_stats.insert_one(stat)
        return True
    except OperationFailure as e:
        logging.error(f"MongoDB operation error adding processing stat: {e}")
        return False
    except Exception as e:
        logging.error(f"An unexpected error occurred adding processing stat: {e}")
        return False

# --- In-memory data storage functions (for child bots only, as per requirements) ---
def register_child_bot_in_memory(token: str, owner_id: str, service_type: str):
    """
    Registers a new child bot in the in-memory data.
    """
    # Per requirements, child bot info is *always* in-memory.
...
def send_subscription_message(chat_id: int):
    """
    Prompt user to join REQUIRED_CHANNEL with the new custom message.
    """
...
def start_handler(message):
    ...
    if not update_user_activity(message.from_user.id):
        bot.send_message(message.chat.id, "Sorry, I'm having trouble connecting to my database. Please try again later.")
        return
    ...
    if message.from_user.id == ADMIN_ID:
        ...
        keyboard.add("Send Broadcast", "Total Users") # Removed "/status"
...
4.  **Privacy & Data Handling**
    ...
    * **Your Settings are Saved:** Your chosen preferences (like selected TTS voice, pitch, rate, and STT language) and basic activity (such as your last active timestamp and usage counts) are stored in our database to provide a consistent experience. This information is used for analytics and to remember your preferences across sessions.
...
At MediaToTextBot, your privacy is our top priority. Here's a quick overview of how we handle your data:

* **No Content Storage:** We **do not store any text you send for TTS or any audio/video files you send for STT**. Your content is processed in real-time and immediately discarded after the conversion is complete. This means your conversations and media remain private to you.
* **Settings Storage:** To provide you with a seamless experience, we store your chosen preferences, such as:
    * Your selected **Text-to-Speech (TTS) voice** (e.g., `en-US-AriaNeural`).
    * Your preferred **pitch** and **rate** for TTS.
    * Your default **Speech-to-Text (STT) language** (e.g., `en`).
    This data allows the bot to remember your settings even if it restarts, so you don't have to set them every time.
* **Usage Statistics:** We collect anonymous, aggregated usage statistics to help us improve the bot's performance and understand user demand. This includes:
    * **Last active timestamp:** To track general bot activity.
    * **TTS conversion count:** The number of times you've used the text-to-speech feature.
    * **STT conversion count:** The number of times you've used the speech-to-text feature.
    This information is purely statistical and does not identify you personally.
* **Child Bots:** If you register a child bot, we store its token, the owner's ID, and the service type (TTS/STT). This is necessary to manage its webhook and functionality. We do not access or store any data that flows through your child bot.

We are committed to transparency and protecting your information. We do not share your data with third parties, except as necessary to provide the bot's services (e.g., sending audio to Microsoft's Azure Text-to-Speech API or audio to AssemblyAI for transcription, which also have their own strict data privacy policies).
...
# Removed @bot.message_handler(commands=['status']) as per requirements.

@bot.message_handler(func=lambda m: m.text == "Total Users" and m.from_user.id == ADMIN_ID)
def total_users(message):
    if not db:
        bot.send_message(message.chat.id, "Sorry, I'm having trouble connecting to my database.")
        return
    ...
    try:
        ...
        logging.error(f"MongoDB operation error counting users: {e}")
        bot.send_message(message.chat.id, "Failed to retrieve total users from the database.")
    except Exception as e:
        logging.error(f"An unexpected error occurred counting users: {e}")
...
def broadcast_message(message):
    ...
    success = fail = 0
    if not db:
        bot.send_message(message.chat.id, "Sorry, I'm having trouble connecting to my database. Cannot broadcast.")
        return
    ...
    try:
        ...
        logging.error(f"MongoDB operation error during broadcast: {e}")
        bot.send_message(message.chat.id, "Failed to retrieve user list from the database for broadcast.")
    except Exception as e:
        logging.error(f"An unexpected error occurred during broadcast: {e}")
        bot.send_message(message.chat.id, "An unexpected error occurred during broadcast.")
...
def process_bot_token(message):
    ...
    # Check if this token is already registered (in-memory, as per requirements)
    if get_child_bot_info_in_memory(bot_token):
        # Directly start the child bot's service if already registered
        service_type = get_child_bot_info_in_memory(bot_token)["service_type"]
        bot.send_message(
            message.chat.id,
            f"This bot token is already registered as a *{service_type.upper()} Bot*! "
            f"You can start using it now: https://t.me/{telebot.TeleBot(bot_token).get_me().username}",
            parse_mode="Markdown"
        )
...
def on_register_bot_service_select(call):
    ...
    if get_child_bot_info_in_memory(bot_token):
        # As per requirements, if token is already registered, just confirm and return.
        service_type = get_child_bot_info_in_memory(bot_token)["service_type"]
        temp_child_bot = telebot.TeleBot(bot_token) # Instantiate to get username
        bot.answer_callback_query(call.id, "This bot token is already registered!")
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                              text=f"This bot token is already registered as a *{service_type.upper()} Bot*! "
                                   f"You can start using it now: https://t.me/{temp_child_bot.get_me().username}",
                              parse_mode="Markdown")
        user_register_bot_mode[uid] = None
        return
...
def handle_rate_callback(call, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    try:
        ...
        if not set_tts_user_rate(user_id_for_settings, rate_value): # Use MongoDB function
            target_bot.answer_callback_query(call.id, "Failed to save speed setting. Database error.")
            return
...
def on_rate_set_callback(call):
    uid = str(call.from_user.id)
    if not update_user_activity(call.from_user.id):
        bot.send_message(call.message.chat.id, "Sorry, I'm having trouble connecting to my database. Please try again later.")
...
def handle_pitch_callback(call, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    try:
        ...
        if not set_tts_user_pitch(user_id_for_settings, pitch_value): # Use MongoDB function
            target_bot.answer_callback_query(call.id, "Failed to save pitch setting. Database error.")
            return
...
def handle_tts_voice_change_callback(call, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    if not set_tts_user_voice(user_id_for_settings, voice): # Use MongoDB function
        target_bot.answer_callback_query(call.id, "Failed to save voice setting. Database error.")
        return

    user_tts_mode[user_id_for_settings] = voice # Update mode with selected voice

    current_pitch = get_tts_user_pitch(user_id_for_settings) # Use MongoDB function
    current_rate = get_tts_user_rate(user_id_for_settings) # Use MongoDB function
...
async def synth_and_send_tts(chat_id: int, user_id_for_settings: str, text: str, target_bot: telebot.TeleBot):
    ...
    voice = get_tts_user_voice(user_id_for_settings) # Use MongoDB function
    pitch = get_tts_user_pitch(user_id_for_settings) # Use MongoDB function
    rate = get_tts_user_rate(user_id_for_settings) # Use MongoDB function
    ...
    try:
        ...
        if not increment_processing_count(user_id_for_settings, "tts"): # Increment user's TTS count via MongoDB
            logging.error(f"Failed to increment TTS count for user {user_id_for_settings}")

        if not add_processing_stat({ # Add stat via MongoDB
            ...
            logging.error(f"Failed to add TTS processing stat for user {user_id_for_settings}")
        ...
        if not add_processing_stat({ # Add stat via MongoDB
            ...
            logging.error(f"Failed to add TTS error stat for user {user_id_for_settings}")
        ...
        if not add_processing_stat({ # Add stat via MongoDB
            ...
            logging.error(f"Failed to add TTS unknown error stat for user {user_id_for_settings}")
    finally:
        stop_recording.set()
...
def handle_stt_language_select_callback(call, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    if not set_stt_user_lang(user_id_for_settings, lang_code): # Use MongoDB function
        target_bot.answer_callback_query(call.id, "Failed to save language setting. Database error.")
        return
...
async def process_stt_media(chat_id: int, user_id_for_settings: str, message_type: str, file_id: str, target_bot: telebot.TeleBot, original_message_id: int): # Added original_message_id
    ...
    status = "failed_unknown" # Default status for stats
    ...
    try:
        ...
        if file_info.file_size > 20 * 1024 * 1024:
            ...
            status = "fail_file_too_large"
            return # Exit function, as nothing else can be done
        ...
        lang_code = get_stt_user_lang(user_id_for_settings) # Use MongoDB function
        ...
        if res['status'] == 'completed':
            ...
            if not increment_processing_count(user_id_for_settings, "stt"): # Increment user's STT count via MongoDB
                logging.error(f"Failed to increment STT count for user {user_id_for_settings}")
            status = "success"
        else:
        ...
        # More specific error for file size handled earlier, this is a general catch-all
        target_bot.send_message(chat_id, "An unexpected error occurred while processing your file. Please try again later.", reply_to_message_id=original_message_id) # Reply to original
        status = "fail_unknown"
        error_msg = str(e)
    finally:
        stop_typing.set()
        ...
        # Always add a processing stat, regardless of success or failure
        processing_time = (datetime.now() - processing_start_time).total_seconds()
        if not add_processing_stat({
            ...
            "file_size": file_info.file_size if 'file_info' in locals() else 0, # Safely get file_size
            "language_code": get_stt_user_lang(user_id_for_settings), # Safely get language_code
            "error_message": error_msg if status.startswith("fail") else None
        }):
            logging.error(f"Failed to add STT processing stat for user {user_id_for_settings}")
...
def handle_stt_media_types_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    """
    if not update_user_activity(int(user_id_for_settings)): # Update this user's activity via MongoDB
        target_bot.send_message(message.chat.id, "Sorry, I'm having trouble connecting to my database. Please try again later.")
        return
    ...
    # This checks against MongoDB, not in_memory_data["stt_settings"]
    if not get_stt_user_lang(user_id_for_settings):
...
def handle_text_for_tts_or_mode_input_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    `user_id_for_settings` is the actual user's ID for settings lookup and activity updates.
    """
    if not update_user_activity(int(user_id_for_settings)):
        target_bot.send_message(message.chat.id, "Sorry, I'm having trouble connecting to my database. Please try again later.")
        return
    ...
    if user_rate_input_mode.get(user_id_for_settings) == "awaiting_rate_input":
        try:
            ...
            if -100 <= rate_val <= 100:
                if not set_tts_user_rate(user_id_for_settings, rate_val): # Use MongoDB function
                    target_bot.send_message(message.chat.id, "Failed to save speed setting. Database error.")
                    return
    ...
    if user_pitch_input_mode.get(user_id_for_settings) == "awaiting_pitch_input":
        try:
            ...
            if -100 <= pitch_val <= 100:
                if not set_tts_user_pitch(user_id_for_settings, pitch_val): # Use MongoDB function
                    target_bot.send_message(message.chat.id, "Failed to save pitch setting. Database error.")
                    return
    ...
    current_voice = get_tts_user_voice(user_id_for_settings) # Use MongoDB function
...
def child_webhook(child_bot_token):
    if request.method == "POST":
        ...
        if content_type and content_type.startswith("application/json"):
            ...
            bot_info = get_child_bot_info_in_memory(child_bot_token) # Child bot info is in-memory
            if not bot_info:
...
def initialize_bot_services():
    ...
    if not connect_to_mongodb(): # Connect to MongoDB
        logging.critical("Failed to connect to MongoDB on startup. Bot will not function correctly.")
        # Optionally, sys.exit(1) here if MongoDB is absolutely critical for startup.
        # For now, allowing partial functionality if in-memory is enough for some parts.
