...
1.  **Text-to-Speech (TTS):** Use /voice to choose a language and voice, then send your text.
    * /pitch: Adjust voice tone.
    * /rate: Change speaking speed.

2.  **Speech-to-Text (STT):** Use /language_stt to set the language, then send a voice, audio, or video file (max 20MB).

3.  **Create Your Own Bot:** Use /register_bot to make a dedicated TTS or STT bot powered by this framework.

4.  **Privacy:** Your content is processed instantly and never stored. Settings are temporary (in-memory).

For questions or issues, contact @user33230.
"""
    )
