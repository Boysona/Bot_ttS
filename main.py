...
class User(Base):
    ...
    tts_conversion_count = Column(Integer, default=0)
    stt_conversion_count = Column(Integer, default=0)
    tts_voice = Column(String, default="so-SO-MuuseNeural")
    tts_pitch = Column(Integer, default=0)
    tts_rate = Column(Integer, default=0)
    stt_language_code = Column(String, default="en")

    def __repr__(self):
        return f"<User(id={self.id}, last_active='{self.last_active}')>"
...
class RegisteredBot(Base):
    ...
    def __repr__(self):
        return f"<RegisteredBot(token={self.token[:5]}..., owner_id={self.owner_id}, service_type='{self.service_type}')>"

class ProcessingStat(Base):
    __tablename__ = 'processing_stats'
    id = Column(Integer, primary_key=True, autoincrement=True)
    user_id = Column(BigInteger)
    type = Column(String) # 'tts' or 'stt'
    processing_time = Column(Integer) # in seconds
    timestamp = Column(DateTime, default=datetime.now)
    status = Column(String) # 'success', 'fail_msspeech_error', 'fail_assemblyai_error', etc.
    details = Column(Text) # JSON string of additional details like voice, pitch, rate, file_size, error_message

    def __repr__(self):
        return f"<ProcessingStat(id={self.id}, user_id={self.user_id}, type='{self.type}', status='{self.status}')>"

# Create all tables in the database
Base.metadata.create_all(engine)

# --- In-memory data storage (for temporary state like input modes) ---
...
# ────────────────────────────────────────────────────────────────────────────────
#   D A T A B A S E   H E L P E R   F U N C T I O N S
# ────────────────────────────────────────────────────────────────────────────────

def get_or_create_user(user_id: int):
    """Fetches a user from DB or creates a new one if not exists."""
    session = Session()
    try:
        ...
        if not user:
            ...
            session.add(user)
            session.commit()
            logging.info(f"Created new user in DB: {user_id}")
        return user
    except SQLAlchemyError as e:
        session.rollback()
        logging.error(f"Database error getting/creating user {user_id}: {e}")
        raise
    finally:
        session.close()

def update_user_activity(user_id: int):
    """Update user last_active timestamp in DB."""
    session = Session()
    try:
        user = session.query(User).filter_by(id=user_id).first()
        if user:
            ...
            get_or_create_user(user_id) # Create if not exists
    except SQLAlchemyError as e:
        session.rollback()
        logging.error(f"Database error updating user activity {user_id}: {e}")
    finally:
        session.close()

def increment_processing_count(user_id: int, service_type: str):
    """Increment conversion count for user in DB."""
    session = Session()
    try:
        user = get_or_create_user(user_id) # Ensures user exists
        if service_type == "tts":
            user.tts_conversion_count += 1
        elif service_type == "stt":
            user.stt_conversion_count += 1
        user.last_active = datetime.now() # Also update activity
        session.merge(user) # Use merge if the user object might be detached
        session.commit()
    except SQLAlchemyError as e:
        session.rollback()
        logging.error(f"Database error incrementing count for user {user_id}: {e}")
    finally:
        session.close()

def get_tts_user_voice(user_id: int) -> str:
    """Return TTS voice from DB."""
    session = Session()
    try:
        user = session.query(User).filter_by(id=user_id).first()
        return user.tts_voice if user else "so-SO-MuuseNeural"
    finally:
        session.close()

def set_tts_user_voice(user_id: int, voice: str):
    ...
    try:
        ...
        session.rollback()
        logging.error(f"Database error setting TTS voice for user {user_id}: {e}")
    finally:
        session.close()

def get_tts_user_pitch(user_id: int) -> int:
    """Return TTS pitch from DB."""
    session = Session()
    try:
        user = session.query(User).filter_by(id=user_id).first()
        return user.tts_pitch if user else 0
    finally:
        session.close()

def set_tts_user_pitch(user_id: int, pitch: int):
    """Save TTS pitch in DB."""
    session = Session()
    try:
        user = get_or_create_user(user_id)
        user.tts_pitch = pitch
        session.merge(user)
        session.commit()
    except SQLAlchemyError as e:
        session.rollback()
        logging.error(f"Database error setting TTS pitch for user {user_id}: {e}")
    finally:
        session.close()

def get_tts_user_rate(user_id: int) -> int:
    """Return TTS rate from DB."""
    session = Session()
    try:
        user = session.query(User).filter_by(id=user_id).first()
        return user.tts_rate if user else 0
    finally:
        session.close()

def set_tts_user_rate(user_id: int, rate: int):
    """Save TTS rate in DB."""
    session = Session()
    try:
        user = get_or_create_user(user_id)
        ...
        session.rollback()
        logging.error(f"Database error setting TTS rate for user {user_id}: {e}")
    finally:
        session.close()

def get_stt_user_lang(user_id: int) -> str:
    """Return STT language from DB."""
    session = Session()
    try:
        user = session.query(User).filter_by(id=user_id).first()
        return user.stt_language_code if user else "en"
    finally:
        session.close()

def set_stt_user_lang(user_id: int, lang_code: str):
    """Save STT language in DB."""
    session = Session()
    try:
        user = get_or_create_user(user_id)
        user.stt_language_code = lang_code
        session.merge(user)
        session.commit()
    except SQLAlchemyError as e:
        session.rollback()
        logging.error(f"Database error setting STT language for user {user_id}: {e}")
    finally:
        session.close()

def register_child_bot(token: str, owner_id: int, service_type: str):
    """Registers a child bot in the database."""
    session = Session()
    try:
        existing_bot = session.query(RegisteredBot).filter_by(token=token).first()
        if existing_bot:
            existing_bot.owner_id = owner_id
            existing_bot.service_type = service_type
            existing_bot.registration_date = datetime.now()
            session.merge(existing_bot)
            logging.info(f"Child bot {token[:5]}... updated in DB.")
        else:
            new_bot = RegisteredBot(token=token, owner_id=owner_id, service_type=service_type)
            session.add(new_bot)
            logging.info(f"Child bot {token[:5]}... registered in DB.")
        session.commit()
        return True
    except SQLAlchemyError as e:
        session.rollback()
        logging.error(f"Database error registering child bot {token[:5]}...: {e}")
        return False
    finally:
        session.close()

def get_child_bot_info(token: str) -> dict | None:
    """Retrieves child bot information from the database."""
    session = Session()
    try:
        bot_info = session.query(RegisteredBot).filter_by(token=token).first()
        if bot_info:
            return {
                "owner_id": bot_info.owner_id,
                "service_type": bot_info.service_type,
                "registration_date": bot_info.registration_date.isoformat()
            }
        return None
    except SQLAlchemyError as e:
        logging.error(f"Database error getting child bot info {token[:5]}...: {e}")
        return None
    finally:
        session.close()

def get_all_registered_bots():
    """Retrieves all registered bots from the database."""
    session = Session()
    try:
        all_bots = session.query(RegisteredBot).all()
        return {bot.token: {"owner_id": bot.owner_id, "service_type": bot.service_type} for bot in all_bots}
    except SQLAlchemyError as e:
        logging.error(f"Database error getting all registered bots: {e}")
        return {}
    finally:
        session.close()

def add_processing_stat(stat: dict):
    """Adds a processing statistic entry to the database."""
    session = Session()
    try:
        # Convert dictionary details to JSON string for storage
        stat_details = stat.copy()
        stat_type = stat_details.pop('type')
        stat_status = stat_details.pop('status')
        stat_processing_time = stat_details.pop('processing_time')
        stat_user_id = stat_details.pop('user_id')
        stat_timestamp = stat_details.pop('timestamp')

        new_stat = ProcessingStat(
            user_id=stat_user_id,
            type=stat_type,
            processing_time=stat_processing_time,
            timestamp=datetime.fromisoformat(stat_timestamp),
            status=stat_status,
            details=json.dumps(stat_details) # Store remaining as JSON
        )
        session.add(new_stat)
        session.commit()
    except SQLAlchemyError as e:
        session.rollback()
        logging.error(f"Database error adding processing stat: {e}")
    except Exception as e:
        logging.error(f"Error preparing processing stat for DB: {e}")
    finally:
        session.close()

def get_total_users_count() -> int:
    """Returns the total number of unique users in the database."""
    session = Session()
    try:
...
def start_handler(message):
    ...
    # Clear in-memory state for user after /start
    user_tts_mode.pop(user_id, None)
    user_pitch_input_mode.pop(user_id, None)
    user_rate_input_mode.pop(user_id, None)
    user_register_bot_mode.pop(user_id, None)
...
4.  **Privacy & Data Handling**
    ...
    * **Your Settings are Saved (in database):** To make your experience seamless, your chosen preferences (like selected TTS voice, pitch, rate, and STT language) are stored in my *database*. This means your settings are remembered even if the bot restarts. We also keep a record of basic activity (such as your last active timestamp and usage counts) for anonymous, aggregated statistics.
...
def total_users(message):
    total_registered = get_total_users_count()
...
def broadcast_message(message):
    ...
    session = Session()
    try:
        all_user_ids = [user.id for user in session.query(User.id).all()]
    except SQLAlchemyError as e:
        logging.error(f"Database error fetching all user IDs for broadcast: {e}")
        bot.send_message(message.chat.id, "Failed to retrieve user list for broadcast.")
        return
    finally:
        session.close()
...
def register_bot_command(message):
    ...
    # Clear in-memory state for user
    user_tts_mode.pop(uid, None)
    user_pitch_input_mode.pop(uid, None)
    user_rate_input_mode.pop(uid, None)
...
@bot.message_handler(func=lambda m: user_register_bot_mode.get(m.from_user.id) == "awaiting_token")
def process_bot_token(message):
    ...
    try:
        ...
        user_register_bot_mode.pop(uid, None)
    except Exception as e:
...
@bot.callback_query_handler(lambda c: c.data.startswith("register_bot_service|") and user_register_bot_mode.get(c.from_user.id) and user_register_bot_mode[c.from_user.id].get("state") == "awaiting_service_type")
def on_register_bot_service_select(call):
    ...
    if not register_child_bot(bot_token, uid, service_type):
        bot.answer_callback_query(call.id, "Failed to save bot registration to database. Please try again.")
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text="An error occurred during registration. Please try again.")
        user_register_bot_mode.pop(uid, None)
        return
...
def handle_rate_command(message, target_bot: telebot.TeleBot, user_id_for_settings: int):
    chat_id = message.chat.id
    user_tts_mode.pop(user_id_for_settings, None)
    user_pitch_input_mode.pop(user_id_for_settings, None)
    user_rate_input_mode[user_id_for_settings] = "awaiting_rate_input"
    user_register_bot_mode.pop(user_id_for_settings, None)
...
def handle_rate_callback(call, target_bot: telebot.TeleBot, user_id_for_settings: int):
    chat_id = call.message.chat.id
    user_rate_input_mode.pop(user_id_for_settings, None)
...
def handle_tts_voice_change_callback(call, target_bot: telebot.TeleBot, user_id_for_settings: int):
    ...
    user_tts_mode[user_id_for_settings] = voice # Keep in memory for current session consistency
...
async def synth_and_send_tts(chat_id: int, user_id_for_settings: int, text: str, target_bot: telebot.TeleBot):
    ...
    try:
        ...
        if not os.path.exists(filename) or os.path.getsize(filename) == 0:
            ...
            status = "fail_empty_audio"
            error_message = "Generated audio file was empty or missing."
            return
...
async def process_stt_media(chat_id: int, user_id_for_settings: int, message_type: str, file_id: str, target_bot: telebot.TeleBot, original_message_id: int):
    ...
    try:
        ...
        if file_info.file_size > 20 * 1024 * 1024:
            target_bot.send_message(chat_id, "⚠️ File is too large. Max size for transcription is 20MB.", reply_to_message_id=original_message_id)
        ...
        target_bot.send_message(chat_id, "An unexpected error occurred during transcription. Please try again later.", reply_to_message_id=original_message_id)
        status = "fail_unknown"
        error_message = str(e)
    finally:
        if processing_msg:
...
def handle_stt_media_types_common(message, target_bot: telebot.TeleBot, user_id_for_settings: int):
    update_user_activity(user_id_for_settings)
    user_tts_mode.pop(user_id_for_settings, None)
    user_pitch_input_mode.pop(user_id_for_settings, None)
    user_rate_input_mode.pop(user_id_for_settings, None)
    user_register_bot_mode.pop(user_id_for_settings, None)
    ...
    # Check if a language is set for STT
    user = get_or_create_user(user_id_for_settings)
    if not user.stt_language_code: # Default is "en", so this might not hit often
...
def child_webhook(child_bot_token):
    if request.method == "POST":
        ...
        if content_type and content_type.startswith("application/json"):
            ...
            if user_id_for_settings is None: # Handle cases where user_id might not be available (e.g., channel posts)
                return "", 200
...
def set_webhook_on_startup():
    try:
        ...
        # Re-set webhooks for all registered child bots from the database
        registered_bots_from_db = get_all_registered_bots()
        for token, info in registered_bots_from_db.items():
...
def set_bot_info_and_startup():
    global bot_start_time
    bot_start_time = datetime.now()
    # init_in_memory_data() no longer needed, data is handled by DB
