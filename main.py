...
def register_child_bot(token: str, owner_id: str, service_type: str):
    ...
    try:
        ...
        # IMPORTANT: This check is not atomic with the insert. A race condition could still lead to DuplicateKeyError.
        if registered_bots_collection.find_one({"_id": token}):
            logging.warning(f"Attempted to register existing child bot token: {token[:5]}... which is already in DB.")
            return False
        ...
        logging.warning(f"Duplicate key error when registering child bot token {token[:5]}...: {e}")
        return False
    except ConnectionFailure as e:
        logging.error(f"MongoDB connection lost during register_child_bot for token {token[:5]}...: {e}")
        return False
    except OperationFailure as e:
        # This will catch other operation issues
        logging.error(f"MongoDB operation failed for register_child_bot for token {token[:5]}...: {e}")
        return False
    except Exception as e:
        logging.error(f"An unexpected error occurred in register_child_bot for token {token[:5]}...: {e}")
        return False
...
def on_register_bot_service_select(call):
    ...
    # Check if this token is already registered (in MongoDB)
    if get_child_bot_info(bot_token):
    ...
    registration_successful = register_child_bot(bot_token, uid, service_type)
    ...
    if registration_successful:
        ...
        bot.answer_callback_query(call.id, "Failed to register your bot in our database.")
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                              text="Failed to register your bot in our database. This might be a temporary issue, please try again later. If the problem persists, contact support.")
