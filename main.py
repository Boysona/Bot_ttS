...
# --- In-memory data storage (only for temporary stats, not persistent user/bot data) ---
in_memory_data = {
...
# Admin state (only if needed for non-persistent admin actions)
admin_state = {}
...
def connect_to_mongodb():
    ...
    Sets global mongo_client and db variables.
    ...
    try:
        ...
        logging.error(f"An unexpected MongoDB error occurred during connection: {e}")
        return False

def get_db_collection(collection_name: str):
    """
    Returns a MongoDB collection. Assumes connect_to_mongodb has been called.
    Returns None if db is not initialized or on error.
    """
    if db is None:
    ...
    try:
        ...
        logging.error(f"Error accessing MongoDB collection '{collection_name}': {e}")
        return None
...
def update_user_activity_db(user_id: int):
    ...
    try:
        ...
        if result.upserted_id:
            logging.info(f"New user {user_id_str} added to DB.")
        return True
    except PyMongoError as e:
        logging.error(f"Error updating user activity for {user_id_str}: {e}")
        return False

def get_user_data_db(user_id: str) -> dict | None:
    """
    Return user document from MongoDB.
    Returns None on error or if user not found.
    """
    users_collection = get_db_collection("users")
    if users_collection is None:
        return False
...
def increment_processing_count_db(user_id: str, service_type: str):
    ...
    try:
        users_collection.update_one(
            ...
            upsert=True # In case user data wasn't created by update_user_activity
        )
        return True
    except PyMongoError as e:
        logging.error(f"Error incrementing processing count for {user_id_str}: {e}")
        return False

def get_tts_user_voice_db(user_id: str) -> str:
    """
    Return TTS voice from MongoDB (default "so-SO-MuuseNeural").
    Returns default voice on error.
    """
    tts_settings_collection = get_db_collection("tts_settings")
    if tts_settings_collection is None:
    ...
    try:
        ...
        logging.error(f"Error getting TTS voice for {user_id}: {e}")
        return "so-SO-MuuseNeural" # Return default on error

def set_tts_user_voice_db(user_id: str, voice: str):
    """
    Save TTS voice in MongoDB.
    Returns True on success, False on failure.
    """
    tts_settings_collection = get_db_collection("tts_settings")
    if tts_settings_collection is None:
...
def get_tts_user_pitch_db(user_id: str) -> int:
    """
    Return TTS pitch from MongoDB (default 0).
    Returns default pitch on error.
    """
    tts_settings_collection = get_db_collection("tts_settings")
    if tts_settings_collection is None:
        return 0
...
def set_tts_user_pitch_db(user_id: str, pitch: int):
    ...
    try:
        ...
        logging.error(f"Error setting TTS pitch for {user_id}: {e}")
        return False

def get_tts_user_rate_db(user_id: str) -> int:
    """
    Return TTS rate from MongoDB (default 0).
    Returns default rate on error.
    """
    tts_settings_collection = get_db_collection("tts_settings")
    if tts_settings_collection is None:
        return 0
    ...
    try:
        ...
        logging.error(f"Error getting TTS rate for {user_id}: {e}")
        return 0 # Return default on error

def set_tts_user_rate_db(user_id: str, rate: int):
    """
    Save TTS rate in MongoDB.
    Returns True on success, False on failure.
    """
    tts_settings_collection = get_db_collection("tts_settings")
    if tts_settings_collection is None:
    ...
    try:
        ...
        logging.error(f"Error setting TTS rate for {user_id}: {e}")
        return False

def get_stt_user_lang_db(user_id: str) -> str:
    """
    Return STT language from MongoDB (default "en").
    Returns default language on error.
    """
    stt_settings_collection = get_db_collection("stt_settings")
    if stt_settings_collection is None:
        return "en"
    ...
    try:
        ...
        logging.error(f"Error getting STT language for {user_id}: {e}")
        return "en" # Return default on error

def set_stt_user_lang_db(user_id: str, lang_code: str):
    """
    Save STT language in MongoDB.
    Returns True on success, False on failure.
    """
    stt_settings_collection = get_db_collection("stt_settings")
    if stt_settings_collection is None:
    ...
    try:
        ...
        logging.error(f"Error setting STT language for {user_id}: {e}")
        return False

def register_child_bot_db(token: str, owner_id: str, service_type: str):
    """
    Registers a new child bot in MongoDB.
    Returns True on success, False on failure (e.g., token already exists or DB error).
    """
    child_bots_collection = get_db_collection("child_bots")
    if child_bots_collection is None:
        return False

    try:
        # Check if token already exists to avoid duplicate entries and enforce unique token
        if child_bots_collection.find_one({"_id": token}):
            logging.warning(f"Attempted to register existing child bot token: {token[:5]}...")
            return False # Indicate that it was not newly registered

        child_bots_collection.insert_one({
            "_id": token, # Use token as _id for uniqueness and easy lookup
            "owner_id": owner_id,
            "service_type": service_type,
            "registration_date": datetime.now().isoformat()
        })
        logging.info(f"Child bot {token[:5]}... registered for owner {owner_id} with service {service_type} in DB.")
        return True
    except PyMongoError as e:
        logging.error(f"Error registering child bot {token[:5]}...: {e}")
        return False

def get_child_bot_info_db(token: str) -> dict | None:
    """
    Retrieves child bot information from MongoDB.
    Returns None on error or if bot not found.
    """
    child_bots_collection = get_db_collection("child_bots")
    if child_bots_collection is None:
        return False
    ...
    try:
        return child_bots_collection.find_one({"_id": token})
    except PyMongoError as e:
        logging.error(f"Error retrieving child bot info for {token[:5]}...: {e}")
        return False

def get_all_child_bots_db() -> list:
    """
    ...
    try:
        return list(child_bots_collection.find({}))
    except PyMongoError as e:
        logging.error(f"Error retrieving all child bots: {e}")
        return []
...
def start_handler(message):
    ...
    if not update_user_activity_db(message.from_user.id):
        bot.send_message(message.chat.id, "Sorry, I'm having trouble connecting to the database. Please try again later.")
        return
...
def total_users(message):
    users_collection = get_db_collection("users")
    if users_collection is None:
        bot.send_message(message.chat.id, "Sorry, I can't retrieve user data from the database at the moment.")
        return
    ...
    try:
        ...
        logging.error(f"Error counting total users: {e}")
        bot.send_message(message.chat.id, "Sorry, I'm having trouble retrieving user count from the database.")
...
def broadcast_message(message):
    ...
    users_collection = get_db_collection("users")
    if users_collection is None:
        bot.send_message(message.chat.id, "Sorry, I can't access the user list for broadcast.")
        return

    try:
        all_users = users_collection.find({}, {"_id": 1}) # Only fetch _id
        ...
        bot.send_message(message.chat.id, "Sorry, an error occurred while fetching the user list for broadcast.")
        return
...
def process_bot_token(message):
    ...
    # Check if this token is already registered in the DB
    existing_bot_info = get_child_bot_info_db(bot_token)
    if existing_bot_info:
        bot.send_message(call.message.chat.id,
                            f"This bot token is already registered and active for *{existing_bot_info['service_type'].upper()}* service!\n\n"
                            f"You can find it here: https://t.me/{telebot.TeleBot(bot_token).get_me().username}",
                            parse_mode="Markdown")
...
def on_register_bot_service_select(call):
    ...
    # Register the bot in MongoDB
    # register_child_bot_db returns False if already exists or on DB error
    if not register_child_bot_db(bot_token, uid, service_type):
        bot.answer_callback_query(call.id, "Failed to register your bot (it might already be registered or a DB error occurred).")
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                text="Failed to register your bot. Please try again later. If it's already registered, try using it.")
        user_register_bot_mode[uid] = None
        return

    # If registration was successful (or if it implicitly started because token already exists)
    try:
        ...
        bot.answer_callback_query(call.id, f"✅ Your {service_type.upper()} bot is registered and ready!")
...
def handle_rate_callback(call, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    try:
        ...
        if not set_tts_user_rate_db(user_id_for_settings, rate_value):
            target_bot.answer_callback_query(call.id, "Database error setting speed.")
            target_bot.edit_message_text(chat_id=chat_id, message_id=call.message.message_id, text="Sorry, there was a database error. Please try again.")
            return
...
def on_rate_set_callback(call):
    uid = str(call.from_user.id)
    if not update_user_activity_db(call.from_user.id):
        bot.answer_callback_query(call.id, "Sorry, I'm having trouble connecting to the database. Please try again later.")
        return
...
def handle_pitch_callback(call, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    try:
        ...
        if not set_tts_user_pitch_db(user_id_for_settings, pitch_value):
            target_bot.answer_callback_query(call.id, "Database error setting pitch.")
            target_bot.edit_message_text(chat_id=chat_id, message_id=call.message.message_id, text="Sorry, there was a database error. Please try again.")
            return
...
def handle_tts_voice_change_callback(call, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    if not set_tts_user_voice_db(user_id_for_settings, voice):
        target_bot.answer_callback_query(call.id, "Database error setting voice.")
        target_bot.edit_message_text(chat_id=chat_id, message_id=call.message.message_id, text="Sorry, there was a database error. Please try again.")
        return
...
async def synth_and_send_tts(chat_id: int, user_id_for_settings: str, text: str, target_bot: telebot.TeleBot):
    ...
    filename = f"tts_audio_cache/tts_{user_id_for_settings}_{uuid.uuid4()}.mp3" # Save to a directory
    ...
    try:
        ...
        increment_processing_count_db(user_id_for_settings, "tts") # Increment user's TTS count

        add_processing_stat_in_memory({ # Still adding to in-memory stats
...
def handle_stt_language_select_callback(call, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    if not set_stt_user_lang_db(user_id_for_settings, lang_code):
        target_bot.answer_callback_query(call.id, "Database error setting language.")
        target_bot.edit_message_text(chat_id=chat_id, message_id=call.message.message_id, text="Sorry, there was a database error. Please try again.")
        return
...
async def process_stt_media(chat_id: int, user_id_for_settings: str, message_type: str, file_id: str, target_bot: telebot.TeleBot, original_message_id: int): # Added original_message_id
    ...
    try:
        ...
        if res['status'] == 'completed':
            ...
            increment_processing_count_db(user_id_for_settings, "stt") # Increment user's STT count
            status = "success"
        else:
...
def handle_stt_media_types_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    """
    if not update_user_activity_db(int(user_id_for_settings)): # Update this user's activity
        target_bot.send_message(message.chat.id, "Sorry, I'm having trouble connecting to the database. Please try again later.")
        return
    ...
    if not get_stt_user_lang_db(user_id_for_settings): # Will return default "en" if no setting
...
def handle_text_for_tts_or_mode_input_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    `user_id_for_settings` is the actual user's ID for settings lookup and activity updates.
    """
    if not update_user_activity_db(int(user_id_for_settings)):
        target_bot.send_message(message.chat.id, "Sorry, I'm having trouble connecting to the database. Please try again later.")
        return
    ...
    if user_rate_input_mode.get(user_id_for_settings) == "awaiting_rate_input":
        try:
            ...
            if -100 <= rate_val <= 100:
                if not set_tts_user_rate_db(user_id_for_settings, rate_val):
                    target_bot.send_message(message.chat.id, "Database error setting speed. Please try again.")
                    return
    ...
    if user_pitch_input_mode.get(user_id_for_settings) == "awaiting_pitch_input":
        try:
            ...
            if -100 <= pitch_val <= 100:
                if not set_tts_user_pitch_db(user_id_for_settings, pitch_val):
                    target_bot.send_message(message.chat.id, "Database error setting pitch. Please try again.")
                    return
...
def child_webhook(child_bot_token):
    if request.method == "POST":
        ...
        if content_type and content_type.startswith("application/json"):
            ...
            # For child bots, the settings always belong to the user interacting with it,
            # not necessarily the owner_id of the child bot.
            owner_id = bot_info["owner_id"] # This is the owner in DB, but user_id_for_settings is the actual user.
            ...
            if message:
                ...
                if message.text and message.text.startswith('/start'):
                    # Ensure user is in DB when they start a child bot
                    if not update_user_activity_db(message.from_user.id):
                        child_bot_instance.send_message(chat_id, "Sorry, I'm having trouble connecting to the database. Please try again later.")
                        return "", 200
                ...
                if message.text:
                    # Update user activity for the interacting user
                    if not update_user_activity_db(message.from_user.id):
                        child_bot_instance.send_message(chat_id, "Sorry, I'm having trouble connecting to the database. Please try again later.")
                        return "", 200
                ...
                # Update user activity for the interacting user
                if not update_user_activity_db(callback_query.from_user.id):
                    child_bot_instance.answer_callback_query(callback_query.id, "Sorry, I'm having trouble connecting to the database. Please try again later.")
                    return "", 200
...
def set_webhook_on_startup():
    try:
        ...
        # Also, set webhooks for all previously registered child bots on startup (from DB)
        registered_child_bots = get_all_child_bots_db()
        if registered_child_bots is False: # Handle DB error
            logging.error("Could not retrieve child bots from DB on startup. Skipping webhook setup for child bots.")
            return

        for bot_info in registered_child_bots:
            token = bot_info["_id"] # Token is the _id in the collection
            service_type = bot_info["service_type"]
...
def initialize_bot_components():
    """
    Initializes all necessary bot components including DB connection and webhooks.
    """
    if not connect_to_mongodb():
        logging.critical("Failed to connect to MongoDB. Bot cannot start.")
        # You might want to exit here or implement a retry mechanism
        exit(1) # Exit if no DB connection
