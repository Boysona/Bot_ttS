...
in_memory_data = {
    ...
    # "registered_bots": {}, # Moved to MongoDB
...
def get_user_data_db(user_id: int) -> dict | None:
    ...
    return in_memory_data["users"].get(str(user_id)) # Fallback to in-memory if DB not available

def update_user_activity_db(user_id: int):
    """Updates user's last_active timestamp and initializes counts in MongoDB or in-memory."""
    ...
    # Also update in-memory for immediate access if needed (though `in_memory_data["users"]` is being phased out for persistent data)
...
def increment_processing_count_db(user_id: int, service_type: str):
    ...
    # Update in-memory as well for consistency, though these counts are now primary in DB
    if user_id_str not in in_memory_data["users"]:
        in_memory_data["users"][user_id_str] = {
            "_id": user_id_str,
...
def set_tts_user_voice_db(user_id: int, voice: str):
    ...
    # Also update in-memory for immediate use without re-querying DB
...
def register_child_bot_db(token: str, owner_id: str, service_type: str):
    ...
    if db:
        ...
        if result.upserted_id:
            ...
            return True # Consider it successfully "registered" or updated
        else:
            logging.warning(f"Child bot {token[:5]}... already registered and no modification needed.")
            return True # Already exists, but we want to allow re-setting webhook
    return False
...
def broadcast_message(message):
    ...
    if db:
        ...
        bot.send_message(message.chat.id, "Broadcast function limited: Database not connected. Only active in-memory users will receive.")
...
async def process_stt_media(chat_id: int, user_id_for_settings: int, message_type: str, file_id: str, target_bot: telebot.TeleBot, original_message_id: int):
    ...
    try:
        ...
        target_bot.send_message(chat_id, "File too large, please send one that is 20MB or smaller.", reply_to_message_id=original_message_id)
        status = "fail_unknown"
        processing_time = (datetime.now() - processing_start_time).total_seconds() if 'processing_start_time' in locals() else 0
        in_memory_data["processing_stats"].append({
            "user_id": str(user_id_for_settings),
...
def child_webhook(child_bot_token):
    if request.method == "POST":
        ...
        if content_type and content_type.startswith("application/json"):
            ...
            bot_info = get_child_bot_info_db(child_bot_token) # Get info from DB
            if not bot_info:
                logging.warning(f"Received update for unregistered child bot token: {child_bot_token[:5]}...")
                return abort(404)

            owner_id = int(bot_info["owner_id"]) # Owner ID for settings lookup (stored as string in DB)
                ...
                if message.text:
                    if service_type == "tts":
                        ...
                        if message.text.startswith('/language_stt'):
                            ...
                            child_bot_instance.send_message(chat_id, "This is an STT bot. Send a voice/audio/video file, or use `/language_stt`.")
                            return "", 200
                    ...
                    child_bot_instance.send_message(chat_id, "Sorry, I can only process specific message types for my service. Check `/start`.")
                    return "", 200
...
def set_bot_commands():
    """Sets commands for the Main bot."""
    commands = [
...
def set_child_bot_commands(child_bot_instance: telebot.TeleBot, service_type: str):
    """Sets commands for a specific child bot based on its service type."""
    commands = []
    if service_type == "tts":
        commands = [
...
def initialize_bot_components():
    global bot_start_time
    bot_start_time = datetime.now()
    connect_to_mongodb() # Connect to MongoDB on startup
