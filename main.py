...
async def process_stt_media(chat_id: int, user_id_for_settings: str, message_type: str, file_id: str, target_bot: telebot.TeleBot, original_message_id: int): # Added original_message_id
    ...
    processing_msg = None
    try:
        # Use reply_to to make the "Processing..." message a reply to the original media
        processing_msg = target_bot.reply_to(message_id=original_message_id, chat_id=chat_id, text="⏳ Processing...")
        ...
        if res['status'] == 'completed':
            text = res.get("text", "")
            if not text:
                target_bot.reply_to(message_id=original_message_id, chat_id=chat_id, text="ℹ️ No transcription returned for this media.") # Reply to original
            elif len(text) <= 4000: # Telegram message limit for reply
                target_bot.reply_to(message_id=original_message_id, chat_id=chat_id, text=text) # Reply to original
            else:
                import io
            ...
            target_bot.reply_to(message_id=original_message_id, chat_id=chat_id, text=f"❌ Transcription error: `{error_msg}`", parse_mode="Markdown") # Reply to original
        ...
        target_bot.reply_to(message_id=original_message_id, chat_id=chat_id, text="❌ A network error occurred while processing your file. Please try again.") # Reply to original
        ...
        target_bot.reply_to(message_id=original_message_id, chat_id=chat_id, text="❌ Oops! An unexpected error occurred during transcription. Please try again in a moment.") # Reply to original
        status = "fail_unknown"
        processing_time = (datetime.now() - processing_start_time).total_seconds() if 'processing_start_time' in locals() else 0
        add_processing_stat_in_memory({
            "user_id": user_id_for_settings, # Use the actual user's ID for stats
