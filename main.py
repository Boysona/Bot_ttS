...
REQUIRED_CHANNEL = "@transcriber_bot_news_channel"  # Updated to match the intended channel
...
def register_child_bot_in_memory(token: str, owner_id: str, service_type: str):
    # Always register or update the bot token, allowing restarts
    in_memory_data["registered_bots"][token] = {
        "owner_id": owner_id,
        "service_type": service_type,
        "registration_date": datetime.now().isoformat()
    }
    logging.info(f"Child bot {token[:5]}... registered/updated for owner {owner_id} with service {service_type} in memory.")
    return True
...
def on_register_bot_service_select(call):
    ...
    if not bot_token:
        ...
        user_register_bot_mode[uid] = None
        return
    # Register or update the bot, always proceeding to set webhook
...
def build_stt_language_keyboard():
    markup = InlineKeyboardMarkup(row_width=3)
    buttons = [InlineKeyboardButton(lang_name, callback_data=f"stt_lang|{lang_code}") for lang_name, lang_code in sorted(STT_LANGUAGES.items(), key=lambda item: item[0])]
    for i in range(0, len(buttons), 3):
        markup.add(*buttons[i:i+3])
    return markup
...
def set_bot_commands():
    commands = [
        ...
        # BotCommand("privacy", "🔒 Read privacy notice"),  # Commented out
    ]
    try:
