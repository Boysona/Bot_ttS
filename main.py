...
# --- BOT CONFIGURATION ---
TOKEN = "8345774316:AAGX_M74WnBjb3Ore6exUtuiEPKXicFgHs0"  # <-- Main Bot Token
ADMIN_ID = 5978150981
...
DB_NAME = "telegram_bot_db" # User wants to use "tokens" database.
...
def connect_to_mongodb():
    ...
    try:
        ...
        db = client[DB_NAME] # User wants to use "tokens" database, so DB_NAME should be "tokens"
        logging.info("Successfully connected to MongoDB!")
    except ConnectionFailure as e:
        logging.error(f"MongoDB connection failed: {e}")
        # Exit or handle error gracefully, maybe retry logic
        exit(1)
...
# --- User state for input modes (still in-memory for immediate session interaction) ---
...
def init_db():
    """Initializes database collections and ensures indexes."""
    logging.info("Initializing MongoDB collections.")
    # Ensure collections exist (they are created on first insert if not)
    # Ensure unique index on _id for users, bots
    db.users.create_index("_id", unique=True)
    db.tts_settings.create_index("user_id", unique=True)
    db.stt_settings.create_index("user_id", unique=True)
    db.registered_bots.create_index("token", unique=True)
    logging.info("MongoDB collections and indexes ensured.")
...
def update_user_activity(user_id: int):
    ...
    db.users.update_one(
        ...
        {"$set": {"last_active": now_iso},
         "$setOnInsert": {"tts_conversion_count": 0, "stt_conversion_count": 0, "registration_date": now_iso}},
        upsert=True
    )
...
def increment_processing_count(user_id: str, service_type: str):
    """Increment conversion count for user in MongoDB."""
    user_id_str = str(user_id)
    now_iso = datetime.now().isoformat()
    update_field = f"{service_type}_conversion_count"
    db.users.update_one(
        {"_id": user_id_str},
        {"$inc": {update_field: 1}, "$set": {"last_active": now_iso}},
        upsert=True
    )

def get_tts_user_voice(user_id: str) -> str:
    """Return TTS voice from MongoDB."""
    settings = db.tts_settings.find_one({"user_id": user_id})
...
def get_stt_user_lang(user_id: str) -> str:
    """Return STT language from MongoDB."""
    settings = db.stt_settings.find_one({"user_id": user_id})
...
def register_child_bot(token: str, owner_id: str, service_type: str):
    """Registers a child bot in MongoDB."""
    try:
        db.registered_bots.update_one(
            {"token": token},
            {"$set": {
                "owner_id": owner_id,
        ...
        logging.error(f"MongoDB error registering child bot: {e}")
        return False
    except Exception as e:
        logging.error(f"Error registering child bot: {e}")
        return False

def get_child_bot_info(token: str) -> dict | None:
    """Retrieves child bot information from MongoDB."""
    return db.registered_bots.find_one({"token": token})
...
def add_processing_stat(stat: dict):
    ...
    try:
        ...
        logging.error(f"Error adding processing stat to MongoDB: {e}")
...
4.  **Privacy & Data Handling**
    ...
    * **Your Settings are Saved (in MongoDB):** To make your experience seamless, your chosen preferences (like selected TTS voice, pitch, rate, and STT language) are stored in my *MongoDB database*. This means your settings are persistent even if the bot restarts. We also keep a record of basic activity (such as your last active timestamp and usage counts) for anonymous, aggregated statistics.
...
def on_register_bot_service_select(call):
    ...
    if not register_child_bot(bot_token, uid, service_type):
        bot.answer_callback_query(call.id, "Failed to register bot in database. Please try again.")
...
def handle_stt_media_types_common(message, target_bot: telebot.TeleBot, user_id_for_settings: str):
    ...
    # Check if STT language is set for the user in the database
    if not db.stt_settings.find_one({"user_id": user_id_for_settings}):
...
def set_bot_info_and_startup():
    ...
    bot_start_time = datetime.now()
    connect_to_mongodb()
    init_db() # Initialize collections and indexes after connection
