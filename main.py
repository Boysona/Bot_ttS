...
1.  **Data We Process & Its Lifecycle:**
    ...
    * **Registered Bots Data:** If you use the `/register_bot` feature, we store the API token of your registered bot, your Telegram User ID as the owner, and the service type (TTS or STT) it provides. This allows our main bot to act as a proxy and process requests for your personal bot. Your bot's token is stored securely.
...
2.  **How Your Data is Used:**
    ...
    * **Child Bot Functionality:** To enable your registered bots to provide dedicated TTS or STT services by routing their requests through our main framework.
...
4.  **Data Retention:**
    ...
    * **Registered Bot Tokens:** Stored as long as your registered bot is active. If you wish to unregister your bot and remove its token, please contact the bot administrator.
...
def handle_stt_media_types(message):
    ...
    threading.Thread(
        target=lambda: asyncio.run(process_stt_media(message.chat.id, uid, message_type, file_id, bot)) # Use main bot for sending
    ).start()
...
def handle_text_for_tts_or_mode_input(message):
    ...
    if current_voice:
        ...
        threading.Thread(
            target=lambda: asyncio.run(synth_and_send_tts(message.chat.id, uid, message.text, bot)) # Use main bot for sending
        ).start()
    else:
...
def child_webhook(child_bot_token):
    if request.method == "POST":
        ...
        if content_type and content_type.startswith("application/json"):
            ...
            if not message:
                return "", 200 # Ignore non-message updates for simplicity

            chat_id = message.chat.id
            # Use the owner_id to retrieve settings, as the child bot is just a proxy
            user_id_for_settings = owner_id

            logging.info(f"Processing update for child bot {child_bot_token[:5]}... (owner: {owner_id}, service: {service_type})")

            # Handle /start command for child bots
            if message.text and message.text.startswith('/start'):
                ...
                if service_type == "tts":
                    ...
                    welcome_message += (
                        ...
                        "Just send me text to get started!"
                    )
                elif service_type == "stt":
                    current_lang = next((name for name, code in STT_LANGUAGES.items() if code == get_stt_user_lang_db(user_id_for_settings)), "English")
                    welcome_message += (
                        f"I transcribe your speech and audio to text. Your current transcription language (managed by the main bot): *{current_lang}*.\n\n"
                        "Send me a voice message, audio, or video file to transcribe!"
                    )
            ...
            if service_type == "tts":
                if message.text:
                    if len(message.text) > 1000:
                        child_bot_instance.send_message(chat_id, "Please keep your text under 1000 characters for text-to-speech conversion.")
                        return "", 200
                    threading.Thread(
                        target=lambda: asyncio.run(synth_and_send_tts(chat_id, user_id_for_settings, message.text, child_bot_instance))
                    ).start()
                else:
                    child_bot_instance.send_message(chat_id, "Please send me text to convert to speech.")
            elif service_type == "stt":
                file_id = None
                message_type = None
                if message.voice:
                ...
                if file_id:
                    threading.Thread(
                        target=lambda: asyncio.run(process_stt_media(chat_id, user_id_for_settings, message_type, file_id, child_bot_instance))
                    ).start()
                else:
                    child_bot_instance.send_message(chat_id, "Please send me a voice message, audio, or video file to transcribe.")
            return "", 200
    return abort(403)
...
def set_webhook_on_startup():
    try:
        # It's good practice to delete existing webhooks before setting a new one
        bot.delete_webhook()
